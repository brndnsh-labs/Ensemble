...
export function generateResolutionNotes(step, arranger, enabled, bpm = 100, groove = {}, soloist = {}) {
    ...
    if (arranger.stepMap && arranger.stepMap.length > 0) {
        ...
        if (lastEntry && lastEntry.chord) {
            ...
            if (lastEntry.chord.key) resolutionKey = lastEntry.chord.key;
    ...
    if (anchor > 68) anchor -= 12;
    if (anchor < 55) anchor += 12;
    ...
    timingMap.forEach((entry, idx) => {
        ...
        if (playRoot - anchor > 6) playRoot -= 12;
        if (anchor - playRoot > 6) playRoot += 12;

        // Shared Voicing Calculation
        const density = (genre === 'Jazz' || genre === 'Neo-Soul') ? 'rich' : 'standard';
        const is7th = cadenceStep.quality.includes('7') || cadenceStep.quality.includes('9') || cadenceStep.quality.includes('13');
        const intervals = getIntervals(cadenceStep.quality, is7th, density, genre, true);
        
        const voicings = getBestInversion(playRoot, intervals, lastMidis, false, anchor, 48, 80, genre === 'Disco' ? 'disco' : 'stabs');
        lastMidis = voicings; 
        ...
        if (enabled.chords) {
            ...
            if (idx === 0) {
                notes.push({
                    midi: 0, module: 'chords', step: step, timingOffset: 0,
        ...
        if (enabled.bass) {
            ...
            if (bassMidi < 33) bassMidi += 12; // Avoid too low
        ...
        if (enabled.groove) {
            ...
            notes.push({ module: 'groove', name: 'Kick', velocity: kickVel, midiVelocity: Math.round(kickVel * 127), step, timingOffset: time });

            if (isLast) {
                if (!cadenceStep.staccato) {
                    notes.push({ module: 'groove', name: 'Crash', velocity: crashVel, midiVelocity: 127, step, timingOffset: time });
                    // Big Rock Ending effect
                    if (genre === 'Rock' || genre === 'Metal') {
                        notes.push({ module: 'groove', name: 'Crash', velocity: 0.9, midiVelocity: 115, step, timingOffset: time + 0.1 });
                    }
                } else {
                    // Staccato funk ending: Snare + Kick + Crash choke
                    notes.push({ module: 'groove', name: 'Snare', velocity: 1.0, midiVelocity: 127, step, timingOffset: time });
                    notes.push({ module: 'groove', name: 'Crash', velocity: 0.8, midiVelocity: 100, step, timingOffset: time });
                }
            } else {
                if (genre !== 'Acoustic') {
                    notes.push({ module: 'groove', name: 'ClosedHat', velocity: 0.6, midiVelocity: 80, step, timingOffset: time });
                    if (cadenceStep.beats >= 2) {
                        notes.push({ module: 'groove', name: 'Snare', velocity: 0.7, midiVelocity: 90, step, timingOffset: time + spb });
                    }
                }
            }
        }
