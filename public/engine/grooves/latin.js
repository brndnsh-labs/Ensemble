...
export function applyOverrides(context, state) {
    ...
    if (inst.name === 'Kick') {
        ...
        if ([0, 3, 8, 11].includes(loopStep)) {
            ...
            velocity =
                loopStep === 0 || loopStep === 8 ? 1.1 + intensity * 0.1 : 0.85 + intensity * 0.1;
        }
        ...
        if (shouldPlay) {
            ...
            if (intensity > 0.85 && Math.random() < 0.4) {
                soundName = 'Snare';
                velocity *= 1.15;
            }
        }
        ...
        if (tumbaoSteps.includes(loopStep)) {
            ...
            if (loopStep === 12) {
                ...
                velocity = 0.8 + intensity * 0.25; // Harder slap with intensity
            } else if (loopStep === 15) {
                soundName = 'CongaHigh';
                velocity = 0.7 + intensity * 0.1;
            } else {
        ...
        if (intensity > 0.8 && (activeMotif === 2 || activeMotif === 3)) {
            if ([3, 6, 11, 14].includes(loopStep) && Math.random() < 0.25 * intensity) {
                shouldPlay = true;
                velocity = 0.9;
