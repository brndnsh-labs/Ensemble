...
vi.mock('../../../public/state.js', () => ({
    getState: () => ({
        ...
        groove: { lastDrumPreset: 'test', measures: 1, swing: 0, swingSub: '8th', instruments: [] }
    }),
    dispatch: vi.fn()
}));
...
describe('Security: JSON.parse Error Handling', () => {
    ...
    it('arranger-controller: saveProgression should handle malformed localStorage gracefully', () => {
        ...
        const saved = JSON.parse(localStorage.setItem.mock.calls.find(call => call[0] === 'ensemble_userPresets')[1]);
    ...
    it('instrument-controller: saveDrumPreset should handle malformed localStorage gracefully', () => {
        ...
        const saved = JSON.parse(localStorage.setItem.mock.calls.find(call => call[0] === 'ensemble_userDrumPresets')[1]);
        expect(Array.isArray(saved)).toBe(true);
        expect(saved.length).toBe(1);
    });
});
