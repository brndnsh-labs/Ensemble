...
import { isExporting, _setExporting } from '../../../public/engine/midi-worker-logic.js';

vi.mock('../../../public/state.js', async (importOriginal) => {
    const actual = await importOriginal();
    const mockChord = { sectionId: 's1', rootMidi: 60, intervals: [0, 4, 7], freqs: [261.63, 329.63, 392.00] };
    const mockState = {
        arranger: { 
            totalSteps: 16, 
            timeSignature: '4/4', 
        ...
        soloist: { enabled: true, volume: 0.5, style: 'smart', buffer: new Map(), phraseContext: {}, busySteps: 0, activeSteps: 0, restSteps: 0, sessionSteps: 0 },
        harmony: { enabled: true, volume: 0.5, style: 'smart', buffer: new Map() },
        groove: { enabled: true, volume: 0.5, measures: 1, instruments: [{ name: 'Kick', steps: new Array(128).fill(0) }] },
...
describe('Logic Worker Messaging', () => {
    ...
    beforeEach(async () => {
        vi.clearAllMocks();
        _setExporting(false);
        LogicWorker = await import('../../../public/logic-worker.js');
    });
    ...
    it('should handle FLUSH message with priming', () => {
        const fullChord = { sectionId: 's1', rootMidi: 60, intervals: [0, 4, 7], freqs: [261.63, 329.63, 392.00] };
        const flushData = {
            step: 0,
            syncData: { 
                arranger: { 
                    ...
                    isMinor: false, 
                    timeSignature: '4/4',
                    measureMap: [{ start: 0, end: 16 }, { start: 16, end: 32 }],
        ...
        const errorCalls = mockPostMessage.mock.calls.filter(c => c[0].type === WORKER_RESP.ERROR);
