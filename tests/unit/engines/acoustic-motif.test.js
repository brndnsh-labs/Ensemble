...
describe('Acoustic Groove - getMotif', () => {
    describe('Low Complexity or Low Intensity (Cajon Feel)', () => {
        ...
        ])('should return motif $expected when seed=$seed, complexity=$complexity, intensity=$intensity', ({ seed, complexity, intensity, expected }) => {
    ...
    describe('Higher Seed Ranges (seed >= 0.5)', () => {
        ...
        describe('Medium Intensity (0.35 <= intensity < 0.7)', () => {
            ...
            it.each([
                { seed: 0.5, expected: 0 },
                { seed: 0.79, expected: 0 },
                { seed: 0.8, expected: 1 },
                { seed: 0.99, expected: 1 },
            ])('should return motif $expected when seed=$seed and intensity=$intensity', ({ seed, expected }) => {
                expect(getMotif(seed, complexity, intensity)).toBe(expected);
            });
        });
        ...
        describe('High Intensity (intensity >= 0.7)', () => {
            ...
            it.each([
                { seed: 0.5, expected: 2 },
                { seed: 0.79, expected: 2 },
                { seed: 0.8, expected: 3 },
                { seed: 0.99, expected: 3 },
            ])('should return motif $expected when seed=$seed and intensity=$intensity', ({ seed, expected }) => {
                expect(getMotif(seed, complexity, intensity)).toBe(expected);
            });
        });
