...
vi.mock('../../../public/state.js', () => {
    const mockState = {
        ...
        groove: { creativity: false, genreFeel: 'Rock', lastDrumPreset: 'Basic Rock', instruments: [] },
...
describe('Groove Engine - Generative (Creativity) Mode', () => {
    ...
    it('should generate extra hits (Entropy) when creativity is enabled', () => {
        ...
        mockParams.step = 1; // Syncopated step
        
        let generatedHits = 0;
        for (let i = 0; i < 200; i++) {
            const result = applyGrooveOverrides(mockParams);
            if (result.shouldPlay) generatedHits++;
        }
    ...
    it('should respect genre boundaries even in creativity mode', () => {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            if (result.shouldPlay && result.velocity < 0.5) entropyHits++; // Entropy hits are low velocity
        }
