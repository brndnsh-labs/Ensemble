...
describe('Coordination Engine', () => {
    describe('createCoordinationContext', () => {
        ...
        it('creates context using provided stepInfo', () => {
            ...
            const context = createCoordinationContext(12, stepInfo as any);
    ...
    describe('updateCoordinationContext', () => {
        it('updates context for soloist module', () => {
            const context = createCoordinationContext(0) as any;
    ...
    describe('enforceRegisterSlotting', () => {
        ...
        describe('Bass Slotting (23 - 57)', () => {
            it('snaps up by octaves if below 23, preserving pitch class', () => {
                const originalMidi = 11; // B
                ...
                expect(result).toBe(23); // 11 + 12
            });

            it('snaps down by octaves if above 57, preserving pitch class', () => {
                const originalMidi = 72; // C
                const result = enforceRegisterSlotting('bass', originalMidi, null);
                expect(result).toBeGreaterThanOrEqual(23);
                expect(result).toBeLessThanOrEqual(57);
                expect(result % 12).toBe(originalMidi % 12);
                expect(result).toBe(48); // 72 - 24
            });

            it('leaves midi unchanged if within 23-57', () => {
