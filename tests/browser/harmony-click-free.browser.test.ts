...
test('harmony voice retires same-MIDI retriggers + cap culls click-free', async () => {
    ...
    // guard — the same idea as `installSeededRandom`, applied where the voice
    // actually runs.
    let seed = 0x9e3779b9;
    Math.random = () => {
    ...
    for (let i = 0; i < 24; i++) {
        ...
        if (stackMidi !== repeated) {
            playHarmonyNote(state, midiToFreq(stackMidi), t + 0.01, 0.3, 0.5, 'stabs', stackMidi);
        }
    }
    ...
    expect(nonFinite, 'output must be finite').toBe(0);
    // Sanity: the render actually produced sound.
    expect(maxAbs, 'render must be audible').toBeGreaterThan(0.01);
