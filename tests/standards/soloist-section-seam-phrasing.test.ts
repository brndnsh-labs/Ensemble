...
describe.each([
    ...
    'SEAM_SEED_2',
    'SEAM_SEED_3',
])('Soloist phrases around form seams (#1020) — seed %s', (seedStr) => {
    it('rests into the change and enters big after a rising boundary', () => {
        ...
        const seed = generateSessionSeed(state, state.arranger, 'smart', BAND_INTENSITY, seedStr);
        ...
        const perform = (seamOn) => {
            ...
            for (const LOOP of LOOPS) {
                for (let s = 0; s < total; s++) {
                    ...
                    // bug this now guards.
                    const coord = seamOn ? { sectionStart: sec.start, sectionEnd: sec.end } : {};
