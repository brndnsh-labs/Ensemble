...
describe('Soloist Disco Critique', () => {
    ...
    it('should pass an authenticity critique for a 128-bar Disco soloist performance', () => {
        const numBars = 128;
        ...
        console.log(`[Note Density]          ${notesPerBar.toFixed(2)} notes/bar (Target: 4.0-15.0)`);
        console.log('------------------------------------\n');

        expect(avgInterval).toBeLessThan(8.0);
        expect(notesPerBar).toBeGreaterThan(4.0);
        expect(notesPerBar).toBeLessThan(15.0);
    });
});
