...
---
<!-- cycle:rendered template=skills/implement.md.tmpl hash=b9b6df7ea2d7 — managed by the-cycle; edit the template, not this file -->
...
6. **Mark it `status:in-progress`:** `gh issue edit "<n>" --remove-label "status:ready,status:in-progress,status:in-review,status:needs-decision,status:needs-ear,status:blocked" && gh issue edit "<n>" --add-label "status:in-progress"`
...
11. **Always-brake check** (§5) — if the diff lands on Track `synth` and genuinely-subjective musical work (no critique-test oracle for the idiom, the Needs-ear stop), destructive data ops (drops/rewrites persisted sessions, share-URL schema, preset data, or a state-slice migration that breaks saved state), the state contract (a `@direct-mutation` outside the sanctioned categories), flag it now; `/review`
...
- **Build abandoned** (not handed to `/review`): roll the label back to `status:ready`
  (`gh issue edit "<n>" --remove-label "status:ready,status:in-progress,status:in-review,status:needs-decision,status:needs-ear,status:blocked" && gh issue edit "<n>" --add-label "status:ready"`) so nothing is stranded mid-flight.

Before building, read the `CLAUDE.md` section matching the story's Track/area (always
§ Musical Logic & Generative Standards for a musical story; § State for state writes) and the
nested guide for the directory you touch — `band/CLAUDE.md` for a musical story,
`public/engine/CLAUDE.md` for a synth story.
