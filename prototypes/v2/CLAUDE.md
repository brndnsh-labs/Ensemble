...
**Merging to `main` releases this app (since #1207, 2026-09-15).** Since the cutover (#1357)
this app IS `https://ensemble.brndn.zip/`: CI builds it at `ENSEMBLE_V2_BASE=/` into the
`ensemble-web` image and the `deploy` job releases that tag to prod and then test on every
merge, asserting each host's public `/build.json` names the commit. It is gated by the required
`v2-checks` context (build + `checks/` Playwright suite) next to `checks` and `e2e-tests`. Treat
a v2 story as live production work: it reaches users, and a red v2 build or E2E blocks the
merge. `../v2-api/` is released by the same job, behind `/api/*` on the same origin.

The cutover was a **hard cut**: there is no v1 at `/` any more and no `/v1/` grace path, and
`/v2/*` redirects to the same path at the root (the one carve-out is `/v2/sw.js`, the tombstone
that retires the beta's worker — `hosting/README.md`). v1's browser data is reachable only
through the v1 import (#1274), which copies one way — v1's songs into this songbook, never a
write back to a v1 key — and old `?s=` links open best-effort (#1279). Accounts are on by
default (#1357); `?accounts=off` is the per-device way out, offered back on the songbook.
...
| Bar editing / runtime bridge | `app/measure-editor.tsx`, `app/section-settings.tsx` (#1374), `lib/form-editing.ts`, `lib/documents.ts` (add/remove bars and sections, section settings), `lib/song-meter.ts` (the song's own meter, #1371, and a section's, #1374 — re-fits only bars whose effective meter changes; blocks, never rounds), `lib/runtime.ts` | Preserve authored/runtime separation and stop/load/sync lifecycle |
| Guest saves and recovery | `lib/repository.ts`, `lib/session.ts` | Not the account store; no implicit guest uploads |
| v1 local-data import (#1274) | `lib/import-v1.ts` (+ the songbook card and the song menu's permanent entry) | Read-only `getItem` view of `ensemble_*`; v1's own normalizers then the canonical codec; per-item digests. Guest songbook only — signed in it still writes there, says so, and points at #1268's "Add this device's songs"; an offer the app opened itself is never shown over an account library. **A signed-in run that actually lands something now opens that #1268 offer itself (#1359)**, scoped to the ids it imported/updated (`computeAdoptCandidates`' optional guest-id filter — same dedup and same account-wide cap), on the account page's own path: OPENING it consults nothing, because this is the musician asking and a standing "Not now" from a sign-in must not silence their own gesture. ANSWERING it records the same per-device answer any answer records (`rememberAdoptionDecision`), which also retires the whole-songbook sign-in offer for that owner — deliberately, since re-asking about the same songs on the next sign-in is the nag the latch exists to prevent; the account page's button stays the way back, and `forgetAdoptionDecision` (account-deletion-only, #1351) is never called here. It opens only while signed in with the account library already downloaded, read from the LATEST render rather than from the one that started the run, and an offer is cleared with the session; without that download the card's pointer is the way through. The v1 session is ONE document, id `v1-session`, that a rerun updates in place — never over a copy edited here, which is decided by the CONTENT digests in `v1SessionMark` (written before the save, so a tab that dies mid-write heals) plus any retained draft. The ledger records what was imported AND what was shown-and-finished-with, so the automatic offer stops re-opening for it; a store failure is never recorded, and the menu entry ignores the ledger entirely. "Not now" is a per-device answer (`hasDeclinedV1Import`) that silences the automatic offer for good |
| Old v1 `?s=` share links (#1279) | `lib/v1-link.ts` (+ the shared-link effect in `app/ensemble.tsx`) | Best effort by decision, not a gate: v1's own share codec decodes the sections (decode only — `public/state/share-codec.ts` is the last reader of v1's link format; nothing writes it since #1358), and #1274's `convertV1` converts them as a saved PROGRESSION against a session record built from the query string, so the labels are escaped ONCE and key/meter/tempo/genre ride the session readers that already exist. What lands is what v1 itself would have loaded from those bytes — v1's own per-lane defaults routed by the genre, NOT the songbook's band; only the soloist's trading and a fallback bpm come from the baseline document. It opens as the SAME unsaved shared draft a `#chart=` link does — fresh random id, no ledger, no session mark, no storage write, because a link is not an import and a `v1-session`/`v1-preset-…` collision would be a data bug. Chords, key, meter and tempo are the promise; `bnd`, `style` and `comp` are not read, `int` is not read (it cannot say whether the sender was on auto energy) and `tmr` is a device preference, and `?seed=` is inert (`randomizeSeed` re-rolls on play). **The entry is decided once per page LOAD** (`sharedLinkHandled` latches before anything is decided, or a later re-render re-reads `window.location` and opens a pasted fragment over live work), in this order: a `#chart=` fragment wins; else a v1 payload, even under an unrelated fragment; else any other non-empty hash keeps the v2 error. Consuming a link strips the hash, the v1 parameters AND `?accounts=` — a share link of either shape must never carry a feature-flag side effect, which is also why `accountsFlagRequest` calls `hasV1SharePayload` |
...
| Offline install / deployment | `scripts/offline.mjs`, `../../hosting/web/`, `../../scripts/deploy-test.sh` | Anonymous shell + verified sounds. Both hosts serve the `ensemble-web` image (this export at `/`), released by tag: the CI `deploy` job on every merge, `scripts/deploy-test.sh` for a branch audition on the test host |
| Where the app is served from (#1354) | `scripts/base-path.mjs`, `lib/base-path.ts` | `ENSEMBLE_V2_BASE` — default `/v2`, `/` for the site root — is the ONE value, so the phase-5 flip is a build flag and not a grep. `next.config.mjs` applies it as Next's `basePath` and republishes it as `NEXT_PUBLIC_BASE_PATH`; `scripts/offline.mjs`, `scripts/serve.mjs` and `checks/fixtures.ts` (`BASE`, `appUrl`) read it through the same normaliser, which throws on anything malformed. Next rewrites what it owns (`next/link`, `next/image`, `_next/*`, the prerendered HTML); a hand-written `fetch`, `caches` key or `serviceWorker.register` path does not, so it goes through `withBase`. The generated worker names its `/api` exclusion explicitly — at `/` the scope prefix matches the whole origin, so the accidental exclusion the account API relies on today disappears. Sound-cache keys carry the base |
...
Test deployment is optional per story and never inferred from a push: `scripts/deploy-test.sh`
releases a pushed branch's `ensemble-web` image to the test host (read the deploy-test skill
first). Identify exact artifact/commit and
