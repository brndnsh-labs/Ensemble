...
import { resolveGenre } from '../../../public/data/smart-genres.js';
...
function linkSession(params: URLSearchParams): Record<string, unknown> {
    ...
    return {
        ...
        // own is the one a picked genre plays (`genreSwing`). Without it a `genre=Jazz` link
        // fell to `sessionBand`'s fallback of 0 and played straight eighths (#1412).
        groove: {
            genreFeel: params.get('genre') ?? undefined,
            ...(genre ? genreSwing(genre.name) : null),
        },
    };
}
