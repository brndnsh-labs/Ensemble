...
- UI metadata and musical behavior are split deliberately: menu/category definitions live in `public/data/instrument-styles.ts`, while the actual generative behavior lives in engine modules such as `public/engine/bass-styles.ts`, `public/engine/chords-styles.ts`, and `public/engine/grooves/`.
