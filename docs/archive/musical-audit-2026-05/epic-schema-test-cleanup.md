...
**Status:** Shipped 2026-05-19. (a) Head-bypass jitter PRNG seeded via `scrambleHash`-keyed `(barIndex, sectionId, headNote.step)` in `soloist.ts`; new `soloist-jitter-determinism.test.ts` (revert-confirmed: un-seeded jitter → 24/85 device-free head-bypass attacks mismatch across two RNG streams; seeded → 0). (b) `soloist-engine-determinism.test.ts` asserts byte-reproducibility under a pinned mulberry32 seed (0 divergences over 1024 steps) with a negative-control test proving 338 divergences on the real RNG. A no-stub determinism test stays blocked on the soloist picker — filed in FOLLOWUPS §F. (c) Engine surfaces `note.source` (`'picker'|'device'`) gated behind `debugSoloist`; new `soloist-bird-picker-chromatism.test.ts` asserts picker-only chromatism > 23% (revert-confirmed: chromatic-neighbor admission gate reverted → 19.3%, well below threshold; post-fix 27.1%). Legacy combined-metric test demoted to a loose sanity check. (d) `tests/utils/seeded-random.ts` shared mulberry32 fixture; consumed by (c) and (e), collapsing per-seed variance (e.g. Evans phrase-end home rate swings 17-72% per seed → single deterministic 43.9% on a 20-seed aggregate). (e) Engine surfaces `note.isPhraseEnd`; new `soloist-evans-cadence-critique.test.ts` filters the Evans cadence metric to phrase-end attacks over a 20-seed aggregate, threshold 41.5% (revert-confirmed: `isEvansCadence` early-exit reverted → 39.3%; post-fix 43.9%). All four new tests 30/30 on the reliability loop. 96 standards files / 573 tests green; typecheck + lint clean. Engine touches are test-mode-gated (`debugSoloist`) — production note schema unchanged.
...
**Status:** Shipped 2026-05-19 (per-chunk commits — a+b / e+f / c / g / h). Seven of the eight sub-items completed; (d) consciously left deferred. Every shipped item was revert-confirmed (the target fix reverted → the test goes red) and music-theory-reviewed on its diff. Highlights:
- **(a)** `accompaniment-imperfect-symmetry-critique.test.ts` — the primary fixture (`sec-verse-A`) seeds `compTargetBarInPhrase=0` so every bar rotates and the cascade `>=` is indistinguishable from `===`. Added a `sec-bb` fixture (target=1): bar 0 stays in the Statement register, bars 1-3 cascade. Revert-confirmed (`>=`→`===` reds the bar-2/3 assertions).
- **(b)** `intro-outro-layering.test.ts` — the old single-Kick "drums NOT muted" test ran through `applyGrooveOverrides`, which never consumes `introBarsElapsed`/`outroBarsRemaining` and so could not catch the regression it claimed to. Replaced with a regression-proof subset assertion on `INTRO_MUTES`/`OUTRO_MUTES` keys (any stray drum key fails) + a Kick/Snare/HiHat bar-scan. Old Kick test removed (subsumed).
- **(c)** `bass-chord-change-approach-critique.test.ts` — the octave-jump metric computed `minDist` over every octave of the chart root, re-folding any ±12 displacement away (the regression was invisible to it). Surfaced the engine's single aimed-at target — `normalizeToRange(nextTarget)` — as an optional `note.approachTargetRoot` field (new 6th `result()` param in `bass-engine.ts`, undefined elsewhere, inert in production). Metric now measures against that exact value; revert-confirmed (+12 → maxDist 13, 62/32 violations).
- **(e)** `conductor-arc-critique.test.ts` — the cool-down 30-seed sweep asserted `target < 0.6` against an engine worst-case of 0.575 (0.025 cushion). Pinned the bound to the jitter constant (`COOLDOWN_CEILING 0.5 + JITTER_HALF_RANGE 0.075`) + explicit max-sample assertion. Revert-confirmed (widening `conductor.ts:510` jitter reds it).
- **(f)** `conductor-arc-critique.test.ts` — every macro-arc test used a Chorus next-section (energy 0.9 > every ceiling), so the section-energy clamp term was never observed. Added a transition into a Verse (energy 0.5) at progress 0.25 — inside the development window [0.40,0.70] — so `targetIntensity` reads `getSectionEnergy` directly. Revert-confirmed (forcing `targetEnergy = macroCeiling` lands 0.70).
- **(g)** new `tests/unit/engine/scheduler-harmony-legato.test.ts` — the epic-harmony-polish S1 legato feature had coverage only on the harmony-engine layer; the `scheduleHarmonies` survivor-retention partition and the `playHarmonyNote` voice-extension branch (~110 of ~134 S1 lines) were untested. New integration test drives two emissions through `scheduleHarmonies` (now `export`ed) against a fake AudioContext, asserts the common-tone voice is the SAME JS object across the chord boundary + carries `lastExtendedAt`. Revert-confirmed on BOTH layers.
- **(h)** `funk-backbeat-presence.test.ts` — added PART 3, an integrated conductor-driven arc (one merged loop: conductor ramp + per-tick backbeat routing). The audit-sketched "32-bar ≥80% Snare" assertion does not revert-confirm (the ramp clears the gate band within ~1.5 bars). Teeth moved to the opening window [0.35, 0.4) — the only band where the S8 `216` gate value decides Snare vs Sidestick (below 0.35 the `funk.ts:255` low-intensity fallback Sidesticks regardless; review premise that the band was [0.3,0.4) corrected during implementation). Revert-confirmed (gate 0.3→0.4 → opening window 0/3 Snare).
- **(d)** Sparse-vibe cell collapse + active-vibe ornament collision — consciously NOT addressed: a pre-existing Epic 3 S2 *engine-behavior* issue, not a test-rigor gap. Kept deferred and re-documented in FOLLOWUPS §F "Not promoted" with a pointer to promote it to an Epic 6 follow-up when the vibe path is next revisited.
- **New follow-up filed:** the conductor macro-arc jitter is an un-named inline literal (`conductor.ts:510`) the S3.e test has to hand-copy — FOLLOWUPS §F (new entries) recommends naming + exporting it. Two music-theory-reviewer P2s on (b) (drum-family denylist → regression-proof subset check) and (c) (comment scope) were fixed inline; one P1 on (h) (thin opening window) was addressed with a minimum-window-size floor + a band-bounds correction surfaced during the fix. typecheck + lint + knip clean; reliability loops 10-15/15 per touched file. Two pre-existing unrelated unit-test failures (`hiphop-integrity`, `metal-shred-integrity`) carried over from S1 — unchanged by this work.
