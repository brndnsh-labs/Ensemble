...
**Implementation note (Build 2c — apex reach, shipped).** The money note is realized in `getSoloistNotePhraseFirst` (the apex-reach block): the theme's own apex (highest seed note in the loop) lifted to a strong, resolved **key tone** — tonic or 5th — up near the register ceiling (`themeApex + ~a sixth`, capped ~86, snapped to {tonic, 5th}). It is **session-fixed** (depends only on the theme + key), so the whole climb genuinely aims at one goal. The apex *step* strains toward it by `reachFraction = developmentDepth / (cyclePeriod − 1)` — 0 on the theme-return loop, **fully landing on the money note at the cycle climax** — layered *on top of* 2b's uniform diatonic sequence (the body climbs in parallel; only the peak stretches to the target). The apex step is exempt from the breath gate (the climax never gets gated out) and **keeps the money note rather than chord-snapping** (a held tonic/5th over the changes = idiomatic pedal climax), with a small velocity boost as it's reached. *Deferred:* a stepwise **run-up** into the apex (approach notes walking to the money note), op variety (inversion/displacement), and per-phrase (vs per-loop) apexes.
