...
**Implementation note (Build 2c — apex reach, shipped; refined after code review).** The money note is realized in `getSoloistNotePhraseFirst`: a strong, resolved **key tone** (tonic or 5th) derived **by construction** as the *highest* such tone that sits a clear third or more above the theme apex and within the soloist ceiling (≤ 88, or ≤ 90 if the apex is already high; if none fits, the reach is a no-op). It is **session-fixed** (theme + key), so the whole climb aims at one goal. The apex *step* is driven **purely by the long-range reach** — `midi = round(themeApex + (moneyNote − themeApex) · reachFraction)`, `reachFraction = developmentDepth / (cyclePeriod − 1)` — verbatim at depth 0 (theme-return loops), **landing exactly on the money note at the climax**, monotonic and never overshooting. (The first cut *stacked* the reach on top of 2b's uniform diatonic lift and guarded it with `moneyNote > midi`; review caught that the uniform lift alone could push a high apex past the capped money note, so the climax landed on an arbitrary tension tone ~half the time. The apex is now governed *only* by the reach — the body of the line keeps the parallel sequence, only the peak follows the target.) The apex is exempt from the breath gate (the climax never gets gated out) and **keeps the money note rather than chord-snapping** (a held tonic/5th over the changes = idiomatic pedal climax), with a small velocity boost as it's reached. The developed body line is folded into register as **one unit** (so the contour never inverts at the ceiling seam), and the soloist's high register is respected **in-engine** (`enforceRegisterSlotting` only lifts notes below 52, never clamps the top). *Deferred:* a stepwise **run-up** into the apex, op variety (inversion/displacement), and per-phrase (vs per-loop, loop-global) apexes.
