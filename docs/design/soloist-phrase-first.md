...
- **Ops can stack toward a peak.** Real development combines (fragment + sequence + augment as it climbs), not one-change-at-a-time. The inherited seeder picks exactly one op per restatement (`:1310-1411`); the live developer should treat single-op as a *floor*, stacking more as the arc intensifies.
