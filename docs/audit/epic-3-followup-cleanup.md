...
### S1 — Funk motif-2 displacement re-weight + re-baseline motif-tier test floors · Model: sonnet · Reviewer: music-theory · ✅ SHIPPED 2026-05-28
- **Shipped:** re-weighted the displacement pick 40/35/25 → **50/35/15** (`funk.ts:198`, threshold `< 0.5 ? 0 : < 0.85 ? 1 : 2`). The Epic 12 S6 B5 1-bar `+2` restructure was left untouched (premise correction surfaced during synthesis: the "sustained 2-bar +2" complaint was already fixed by B5; this story de-weights the *pick frequency* of the now-1-bar gesture). Re-set both loose floors against a 20-run sample: funk `barsWithBeat1/3Displacement >= 5` → `>= 8` (observed min 11, 3-bar cushion); hiphop `burstBars >= 5` → `>= 18` (observed 26, 31% headroom). Reviewer: 0 P0 / 0 P1 / 1 P2 (stale "40%" comment, patched inline). 30/30 reliable; spine backbeat clean at 91% of bars.
...
### S5 — Odd-meter dub: grouping-pulse onsets · Model: sonnet · Reviewer: music-theory · ✅ SHIPPED 2026-05-28
- **Shipped:** unified the dub felt-onset selector to `hasGrouping = (grouping.length ?? 1) > 1 ? isPulseStart : isPulse` (`bass-styles.ts:399-402`). 16th-grid odd meters now sit on the grouping pulse (5/4 → {0,12}, 7/4 → {0,16}) instead of a quarter-note pedal; 6/8 {0,6}, 7/8 {0,4,8}, 3/4 (every quarter), 4/4 all byte-identical. One Drop in odd meters = 1 onset/bar (mStep 12/16 — the idiomatic "drop," kept). Pinning test updated to assert the tighter grouping-pulse positions (would fail on old code) + two exact-position cases. Reviewer: **0 P0 / 0 P1 / 0 P2** — "musically correct, deferred-from-S9 quarter-pedal genuinely fixed." 30/30 reliable; typecheck clean.
...
### S10 — Latin generic 6/8 bell pattern · Model: opus · Reviewer: music-theory · ✅ SHIPPED 2026-05-29
- **Shipped:** authored the standard 7-stroke Bembé bell (maximally-even E(7,12), eighth-pulses 0,2,4,5,7,9,11 → IOI **2-2-1-2-2-2-1**, box `101011010101`) on the `AgogoHigh` agogô voice via a top-level `if (isCompound)` branch in the Snare block (`grooves/latin.ts:105`) that supersedes the (correctly `!isCompound`-gated) son-clave / Samba / Partido-Alto motifs. 12/8 = one-bar timeline (steps 0,4,8,10,14,18,22); 6/8 = 2-bar phrase (bar-1 0,4,8,10 / bar-2 2,6,10). Principal strokes (beats 1 & 4) accented above the syncopated middle. Paired-site guards: line-258 low-intensity Sidestick remap gated `!isCompound`; turnaround crack reimplemented inline in the early-return branch. Critique `compound-reggae-latin-critique.test.ts` (c)/(e)/(f) assert the exact positions **and** reconstruct the 12-pulse timeline to assert the IOI signature (guards the idiom, not just the engine's output). 862/862 standards green, 15/15 reliable, typecheck clean.
- **Reviewer (music-theory, opus):** caught a **P0 theory error** — the implementer first built `0,2,3,5,6,8,10` (IOI 2-1-2-1-2-2-2, tracks beats 1/2/3, drops beat 4), which is *not* the standard bell nor a rotation of it. Corrected to the real maximally-even pattern. The reviewer's own 6/8 bar-2 fix had an arithmetic slip (`0,2,6,10`, 4 strokes) — verified against source and corrected to `2,6,10` ([[verify-reviewer-findings]]). P1 accent + P1 tautology + P2 nit all addressed in the same patch.
- **Reachability finding (owner call 2026-05-29):** mid-cycle the owner noted Latin/Afro-Cuban aren't selectable in the UI. Traced: the genre picker (`SMART_GENRES`, 13 genres) exposes **Bossa** as the only Latin-family option (`feel='Bossa Nova'`, loads the `Bossa Nova` drum preset). The generic `'Latin'` strategy key and the `Samba` / `Afro-Cuban 6/8` / `Latin/Salsa` / `Afrobeat` World/Latin drum presets are referenced by no genre and no drum-preset picker → unreachable in production. Owner disposition: **intentional curation** — Bossa is the deliberate single Latin-family genre; the rest stay dormant-by-design (documented in-code at `groove-engine.ts` strategies map; see FOLLOWUPS §H). So this bell's only live trigger today is a **Bossa selection forced to a compound meter** (the picker stars 4/4 for Bossa but allows the pairing); correct + future-proofed for if the bank is ever exposed. **Listening gate deferred** to that exposure rather than blocking the commit — the audition path is itself an edge case and the engine/test correctness is verified.
