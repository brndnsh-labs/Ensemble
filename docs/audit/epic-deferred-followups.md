...
**Status:** Done 2026-05-20 — shipped. All 16 items completed across 12 files, committed per-file (~11 commits). Notable per-item resolutions: item 5 `busySteps`→`coordination.soloistBusy` is a documented superset migration (also fires on short sub-step soloist notes) — WHY comment notes the widened trigger; item 9 (duplicate reggae-harmony case) resolved with an accept-the-duplication comment, not extraction (the two files exercise independent engine entry points, only constant arrays overlapped); item 11 declared `SrdcPhase` + `srdcState?` on `SoloistState` with `@test-only` JSDoc but left the picker's `soloistState: any` param as-is (it performs `@worker-mutation` writes against `readonly` fields a strict type would reject — documented inline). Item 10 `instHash` now routed through canonical `scrambleHash` (reviewer confirmed no dead ceiling gate — `instHash` is only XOR-folded into a fresh `scrambleHash` `targetSeed`). Reviewer: no P0/P1; 1 P2 + 1 nit (both doc-only) patched inline. **8 pre-existing test failures** (`soloist-motivic-response`, `soloist-seeder-hook-shape`, `soloist-trading`, `hiphop-integrity`, `metal-shred-integrity`, `session-arc`) confirmed baseline-matched — unrelated to S5; `hiphop-integrity`/`metal-shred-integrity` already tracked by S8(e).
...
**Status:** Done 2026-05-20 — shipped, committed per sub-item. (a) `getBestInversion`'s 10-positional-arg signature folded to an `InversionOptions` object; production `parseProgressionPart` now passes `enableVoiceLeading: true` gated on `genreFeel ∈ {Jazz, Bossa Nova, Blues}` (the guide-tone/common-tone idioms); `harmonies.ts`/`resolution.ts` migrated to the options form with the flag off — `jazz-comping-integrity.test.ts` proves the production path yields the same-MIDI common-tone hold across Dm7→G7. **Listen-test outstanding** (an agent cannot perform it). (b) Funk Clav now built by pitch-class identity — `pickClavDegree` selects real chord tones for the gapped 3+b7+9 cell rather than handing the full chord to `selectCompactCluster`'s contiguous-window picker (which could only ever return the {3,5,b7} dominant shell); reviewer P0. (c) plain major triads in `Acoustic`/`Neo-Soul`/`Country` get a deterministic add9 at `intensity ≥ 0.35` (was `≥ 0.6`) — audit's "Pop"/"R&B" sketch keys aren't canonical `genreFeel` values, used the actual color-friendly genres. (d) country boom-chick bass leg lifts by octaves to clear `max(52, bassMidi+5)` when a band bassist is present, capped at the 84 chord ceiling. Reviewer: P0 (funk Clav never produced the gapped cell) + P1 (funk test asserted note count not content) + 2×P2 — all patched; re-review caught one latent theory error (`/^m/` wrongly flagged maj7 as minor) — patched to the canonical `startsWith('m') && !startsWith('maj')` predicate; re-review clean. Pre-existing follow-up flagged: `bass-chord-change-approach-critique.test.ts` has an under-cushioned unseeded stochastic threshold (~1pp margin) — filed to FOLLOWUPS.
...
**Status:** Done 2026-05-20 — shipped, committed per sub-item. (a) New exported pure `pickByRank<T>` helper in `soloist-pitch-engine.ts` (linear rank weighting: rank 0 → weight N, last → weight 1) replaces the uniform `fittedAllowed[Math.floor(Math.random()*len)]` device pick — final-stage selection over the already-best-first ranked list, worst device kept non-trivially alive (~5-7% at N=5/6) so variety survives. (b) Explicit `halfdim` (→ nat-3 / rootPc+4, locrian-bebop bridging b3↔4) and `augmaj7` (→ b7 / rootPc+10, lydian-aug bridging 6↔maj7) branches added to the `bebopScale` passing-tone conditional — the old `augmaj7`-into-major fold put the passing PC on the #5 chord tone, degenerating the walk to a chromaticism-free scalar line. (c) Reggae coordination-fill phrase-end-only branch now walks in from a diatonic scale tone of the current chord (9-above / b7-below) instead of a chromatic ±1 rub; the chord-CHANGE branch (chromatic approach into a *new* root — correct functional voice-leading) is untouched. Reviewer: 0 P0, 1 P1 (a test-comment cited a non-reproducing "15.5-22.8%" band for the consequent `themed-improv jitter` threshold widen — re-measured over 30 runs at 7.0-20.3%, threshold corrected 0.30→0.27 with reproducible numbers), 2 P2 (recorded no-regression check; reggae major-quality walk-in coverage gap — clarified the maj7-leading-tone case is diatonic in a test comment). `pickByRank` is exported solely for deterministic test measurement and is the ready injection point if FOLLOWUPS §F migrates the soloist picker to `scrambleHash`.
...
**Status:** Done 2026-05-20 — shipped, committed per sub-item. (a) `Reggae` FILL_TEMPLATES entry (Carlton Barrett One-Drop tom-downs resolving soft toward the silent beat-1) + two `Ska-Punk` tom templates (Barker/Armstrong H→M→L descent + snare/tom blitz); `tom-vocabulary-critique.test.ts` extended to cover both. (b) New `accentCymbal: 'Crash' | 'China'` groove-config field (`DEFAULT_CONFIG` `'Crash'`; Metal declares `'China'`, Shred inherits); threaded through the post-turnaround section-boundary splash + soloist crash-catch accent in `groove-engine.ts`. (c) `applyGrooveOverrides` call in `tick-logic.ts` now passes `mStep`/`stepInGroup`/`groupIndex`/`isCompound`/`isPulse`/`isPulseStart` from `stepInfo` — they were declared on `GrooveOverrideOptions` but never passed, so per-genre strategies silently consumed `undefined`; re-activates previously-dead idiom branches in jazz/funk/latin/reggae (reviewer P2 — WHY comment broadened to name the wider blast radius). (d) Conga/Bongo namespace reconciled on suffix-first (`CongaHigh`/`BongoLow`) across `DRUM_MAP`, `KNOWN_SOUND_NAMES`, and the MIDI lookup. (e) The two named failures (`hiphop-integrity`, `metal-shred-integrity`) were confirmed stale fixtures, not engine bugs — `sectionSeed 0.5→0.31` (avoids the S6 trap-roll burst beat) and `isOffbeat→isBeatStart` (matches the drums.md P1 #6 blast-beat alternation contract); engine code untouched. Reviewer: 0 P0/P1, 1 P2 + 1 NIT (both comment-accuracy, patched inline). Follow-ups flagged: DRUM_MAP `Agogo` keys have the identical space-form/suffix-first asymmetry (scoped out — Conga/Bongo only); the final-bar resolution Crash (`groove-engine.ts:535`) also hard-codes `'Crash'` and Metal would arguably want China there too; 5 unrelated pre-existing soloist/chords PRNG-drift test failures remain red on `main` (incl. `soloist-motivic-response`, which S10's status claims fixed — flagged for diagnosis). Listen-test (b) China splash outstanding.
...
**Scope correction (during implementation, 2026-05-20).** The implementation pass found the diagnosis above was two-thirds right and the real loop-1 anchor mechanism is subtler — the clobbering device is a *carried-over* `bluesTurnaround` `deviceBuffer` (16 steps) created near the end of loop 0, drained at the top of `getSoloistNote` before the head-bypass path runs. Fixed by discarding stale device/embellishment buffers at loop-1 anchor steps (`soloist.ts:965-991`), plus the `deviceBaseProb = 0` kill and turnaround-substitution suppression originally sketched. Separately, the **Neo-Soul `headC.richContourShare`** failure was confirmed *not* a loop-1 anchor defect at all — it is a melodic-contour metric on a loop-0 seed sweep, an independent RNG-stream-drift artifact from `008b2400` (cf. the PRNG-stream-drift memory note). It shares only a *trigger commit* with the anchor failures, not a defect. **Split out of S10** → FOLLOWUPS (soloist Neo-Soul contour re-baseline; a test-threshold decision, not an engine fix).

**Acceptance:** loop-1 anchor head notes emit their exact head pitch independent of RNG-stream position; `soloist-motivic-response` passes (both the aggregate and single-seed `blues loop1AnchorExactRate`); new stream-alignment-independent critique coverage (`tests/standards/soloist-loop1-anchor-fidelity.test.ts` — per-seed exact-rate guarantee across 6 head seeds); `laterLoopAnchorExactRate` stays ≥ its current thresholds. The Neo-Soul `soloist-seeder-hook-shape` `richContourShare` case is **out of scope** (split to FOLLOWUPS). Listen-test pass (the paraphrase loop should still feel like a paraphrase, not a mechanical head repeat) — passed 2026-05-20.
...
**Status:** Done 2026-05-20 — shipped + **listen-test passed**. Loop-1 anchor `deviceBaseProb = 0` kill + blues-turnaround-substitution guard (`soloist-pitch-engine.ts`); carried-over device/embellishment buffer discarded at loop-1 head-anchor steps with residual `busySteps` cleared (`soloist.ts`); loops 2+ deliberately untouched (existing anchor `×0.35` holds `laterLoopAnchorExactRate` ~80%). New stream-independent critique test asserts loop-1 anchor exact pitch as a categorical guarantee (`=== 1`) per seed across 6 head seeds; 30/30 reliability loop. Neo-Soul `richContourShare` split to FOLLOWUPS. Reviewer P0 was the scope question (resolved by the split); P2 (test threshold underselling a 100% guarantee) patched to equality.
