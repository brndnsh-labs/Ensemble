...
**Status:** Done 2026-05-20 — code shipped; **listen-test pending user verification**. Section-boundary lookahead + `drop-mechanic.ts` gate (genre + strict-`>` energy-delta threshold, float-noise margin so canonical pre-chorus→chorus does not fire); 1-bar band-wide cut + downbeat crash + slam-back; `drum-seeder.ts` no longer aliases `drop`→`Chorus`. New `drop-breakdown-mechanic.test.ts` (11 cases). Trigger keys on `upcomingSection*` only — a chart that opens on a Drop is a documented non-goal. Breakdown reuses the Drop gesture by decision. Reviewer P2 (CoordinationContext typing) deferred to FOLLOWUPS §G.
