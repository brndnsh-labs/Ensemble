...
- 📌 PROMOTED — **Epic 3/S8** (owner decision 2026-05-28: lower floor to 0.25 — but INVESTIGATE FIRST; the `intensity ≥ 0.4` gate could not be located in live source during synthesis, the gesture was refactored since Epic 2 S2). **Imperfect Symmetry intensity 0.4 floor.** Epic 2 S2 gates the gesture at `intensity ≥ 0.4`, suppressing it during quiet ballad-style Verse 2 — exactly where subtle variation is most musical. Consider 0.25 or gentler upward bias at low intensity. ~1h. *Source: Epic 2 S2 review.*
...
- 📌 PROMOTED — **Epic 3/S2** (owner decision 2026-05-28: make the 6th chord-quality aware). **Walking-ska M6 over minor chords.** `bass.md` P1 #9. Small follow-on. ~1h. *Source: Epic 5 Notes.*
- 📌 PROMOTED — **Epic 3/S11** (owner decision 2026-05-28: add next-chord-root target-awareness on the final beat(s)). **Generic walking target-awareness.** `bass.md` P1 #10. ~2h. *Source: Epic 5 Notes.*
- 📌 PROMOTED — **Epic 3/S4** (owner decision 2026-05-28: document the probability values; verify the §G.17 seeding WHY-block doesn't already cover them). **Funk pop/chuck/hammer probability documentation.** `bass.md` P2 #17. Doc/comment pass. ~1h. *Source: Epic 5 Notes.*
- 📌 PROMOTED — **Epic 3/S3** (owner decision 2026-05-28: fix the fallback to step to the nearest in-scale tone). **`findNextBebopMidi` whole-tone fallback.** `soloist-devices.ts` bebopScale branch's `findNextBebopMidi` falls back to `from + stepDir * 2` (a whole step) when no bebop-set PC is found within 4 semitones — only triggers on degenerate scales (whole-tone, diminished). The fallback steps a fixed whole tone regardless of which scale; for whole-tone scales it stays inside the scale (no-op), for diminished it lands on a non-scale tone. NIT-level; never observed in jazz-style runs. ~30min. *Source: Epic 4 / S3 review 2026-05-17.*
