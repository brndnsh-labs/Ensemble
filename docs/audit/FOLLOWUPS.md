...
- **Three pre-existing soloist motivic-fidelity test failures.** Confirmed pre-existing by checking out `b97030b5` (the pre-Epic-11 commit) and re-running — all three fail there too, so they predate the Epic 11 work and are not S5 regressions. (a) `tests/integration/soloist-motivic-response.test.ts` — "keeps later-loop paraphrases recognizable across core hook styles": `blues.aggregate.loop1AnchorExactRate` observed ~0.667, expects ≥ 0.7. (b) Same file — "keeps the audit head seed anchored and triplet-aware on later loops": `blues.loop1AnchorExactRate` observed ~0.25, expects ≥ 0.75 (large gap — likely a real engine drift, not a marginal threshold). (c) `tests/integration/soloist-seeder-hook-shape.test.ts` — "keeps Neo-Soul HEAD_C from collapsing into a narrow scalar lane": `headC.richContourShare` observed ~0.5625, expects ≥ 0.6. All three are blues/Neo-Soul HEAD-anchor + later-loop-fidelity metrics; the (b) failure's size suggests a soloist seeder/anchor regression that slipped past the suite at commit time rather than stale fixtures. ~1-2h to diagnose whether the engine drifted or the fixtures went stale; fix the side that's wrong. *Source: Epic 11 S5 cycle (2026-05-20).*
