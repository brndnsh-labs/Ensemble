...
## Open count (reconciled 2026-05-28)

**2026-05-28 reconciliation (Epic 3 synthesis):** a one-at-a-time owner triage of the open items promoted **12** to [`epic-3-followup-cleanup.md`](epic-3-followup-cleanup.md) (marked `📌 PROMOTED — Epic 3/S<N>` inline below) and found **6 already shipped** (marked `✅ CLOSED` below — the 2026-05-26 count predated Epic 12 S6/S10 and the two meter cycles): final-bar HiHat gate, conductor ramp 0.75/1.25, Ska 0.4 floor, Ska antiphony wire-up, China 1.0, plus the per-genre final-bar drum work. One item (per-genre intro/outro mutes) is `🎧 DEFERRED TO LISTENING SESSION`. Left-as-is by decision: comping color RNG (variety is intended), §B.2 device-floor ceiling (documented limit), non-jazz walking-bass compound pickers (latent), Disco intensity-axis. Lesson banked: [[followup-sweep-triage]] — the stale open-count block nearly spawned 6 dead stories; always grep live source first.

The pre-2026-05-28 snapshot below is retained for context.
...
- ✅ CLOSED (already shipped — verified stale 2026-05-28): **HiHat suppression on final bar reads abrupt in 8th-note-hat genres.** The per-genre gate exists — `groove-engine.ts:809/840` gates the final-bar hat on `!HAT_SPINE_GENRES.has(genreFeel)` (Epic 12 S6 B6). The owner's "per-genre gate" decision was already implemented. *Source: Epic 2 S4 review; closed via Epic 12 S6 B6.*
- ✅ CLOSED (already shipped — verified 2026-05-28): promoted to Epic 3/S8 then found already done by **drain S6 B1** (`118c5018`) — bass Imperfect Symmetry floor lowered `0.4 → 0.25` at `bass-engine.ts:451`, exactly the value chosen. (Synthesis couldn't locate it because it grepped soloist/groove paths, not `bass-engine.ts`.) **Imperfect Symmetry intensity 0.4 floor.** Epic 2 S2 gates the gesture at `intensity ≥ 0.4`, suppressing it during quiet ballad-style Verse 2 — exactly where subtle variation is most musical. Consider 0.25 or gentler upward bias at low intensity. ~1h. *Source: Epic 2 S2 review.*
- 🎧 DEFERRED TO LISTENING SESSION (owner decision 2026-05-28: no single "correct" stagger — decide by ear per genre, not a blind value change). **Per-genre intro/outro mute tuning.** Epic 2 S5 currently genre-flat (`INTRO_MUTES = { bass: 2, chords: 3, harmony: 4 }`). ~3h. *Source: Epic 2 S5 Deferred.*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): **S8 ramp-inversion aggressiveness.** Already softened to `0.75 down / 1.25 up` (`conductor.ts:~272`, Epic 12 S6 / LISTEN_TESTS B2) — the exact value the owner chose. Original entry below for grep-from-finding: `conductor.ts:229` ships `0.5 down / 1.5 up`; with `stepSize = (target - current)/16` the up-ramp can leap +0.25 in a single measure (verified in S8 trace: `0.50 → 0.75`). Effectively trades the pre-S8 floor-bias for a ceiling-bias. Listen-test alternative: `0.75 / 1.25` (gentler) or `1.0 / 1.0` (neutral baseline). Audit doc S8 explicitly said "Pick after a listen-test of both directions"; the shipped value is plausible but unverified by listening. ~1h. *Source: form-arrangement/S8 review (2026-05-17).*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): **S8 Ska-Punk genre floor.** Already added — `GENRE_INTENSITY_FLOORS` has `Ska: 0.4` (`conductor.ts:~67`, meter-robustness S1, keyed on the canonical `'Ska'` feel) — the exact value the owner chose. Original entry: S8 lowered the Ska-Punk backbeat Snare gate to 0.3 but did NOT add a `GENRE_INTENSITY_FLOORS` entry. Ska-Punk is high-energy by genre identity (the comment in `ska-punk.ts:155` says so); should get a floor around 0.4 analogous to Disco 0.45 to keep the upbeat-crack consistent. Inconsistent calibration story with Funk (which got gate + floor). ~30min. *Source: form-arrangement/S8 review (2026-05-17).*
...
- ✅ CLOSED (already shipped — verified stale 2026-05-28): promoted to Epic 3/S2 then found already done by **drain S5 / Epic 12 S5** (`8ac896ef`) — `bass-styles.ts:1338` already does the scale-aware sixth pick. **Walking-ska M6 over minor chords.** `bass.md` P1 #9. Small follow-on. ~1h. *Source: Epic 5 Notes.*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): promoted to Epic 3/S11 then found already done by **drain S5 / Epic 12 S5** (`8ac896ef`) — `bass-engine.ts:1191-1265` target-distance multiplier. **Generic walking target-awareness.** `bass.md` P1 #10. ~2h. *Source: Epic 5 Notes.*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): promoted to Epic 3/S4 then found already done by **drain S4** (Shipped 2026-05-23) — slap gates carry inline probability comments + the §G.17 seeding WHY-block. **Funk pop/chuck/hammer probability documentation.** `bass.md` P2 #17. Doc/comment pass. ~1h. *Source: Epic 5 Notes.*
- ✅ CLOSED (moot — verified 2026-05-28): promoted to Epic 3/S3 then found documented won't-fix by **drain S4** — `soloist-devices.ts:595-614` proves the fallback is unreachable for whole-tone AND diminished (loop matches at semi≤2 first); fires only on pathological <3-note scales where no correct tone exists. **`findNextBebopMidi` whole-tone fallback.** `soloist-devices.ts` bebopScale branch's `findNextBebopMidi` falls back to `from + stepDir * 2` (a whole step) when no bebop-set PC is found within 4 semitones — only triggers on degenerate scales (whole-tone, diminished). The fallback steps a fixed whole tone regardless of which scale; for whole-tone scales it stays inside the scale (no-op), for diminished it lands on a non-scale tone. NIT-level; never observed in jazz-style runs. ~30min. *Source: Epic 4 / S3 review 2026-05-17.*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): **China cymbal `volumeScale` recalibration after triple-stack fix.** Already raised to `1.0` (`synth-drums.ts:467`, Epic 12 S6 B4 — "raised from 0.85, the original defensive") — China now reads above the Crash, which is what the owner's 0.95 decision was reaching for. Original entry: `synth-drums.ts` China runtime profile ships `volumeScale: 0.85` — picked to trim slightly under Crash's 0.90 as defensive headroom against the metal.ts triple-stack that fired three China voices per downbeat. After the Epic 7 S5 P0 fix scopes the accent to the Open lane only, that justification no longer holds. A real Holy China / Mb20 Trash typically peaks *above* the Crash in a kit; 0.85 leaves China reading quieter than the Crash it replaces. Listen-test 0.90 / 0.95 / 1.0 against Crash at the same accent and pick by ear. ~30min. *Source: drums-idiom/S5 review (2026-05-18).*
- 📌 PROMOTED — **Epic 3/S1** (owner decision 2026-05-28: re-weight to 50/35/15 — `< 0.5 ? 0 : < 0.85 ? 1 : 2`). **Funk motif-2 `+2` displacement frequency may be too high.** `grooves/funk.ts:184` ships `< 0.4 ? 0 : < 0.75 ? 1 : 2` — 25% of motif-2 phrases land on `+2` (both backbeats shifted to & of 2 / & of 4 for a sustained 2-bar phrase). Stubblefield/Garibaldi displacement is far more often the laid-back `+1` (e of backbeat); the full `+2` substitution is canonically a 1-bar fill setup, not a sustained groove. Consider 50%/35%/15% (normal-heavy) or restructure `+2` as a 1-bar gesture that returns to normal next bar. Listen-test required. ~1h. *Source: drums-idiom/S6 review (2026-05-18).*
- 📌 PROMOTED — **Epic 3/S1** (owner decision 2026-05-28: re-baseline both floors in the same commit as the funk displacement re-weight, after a 20-run sample). **Funk + Hip-Hop motif-tier test floors very loose vs expected rate.** S6 critique tests pin `barsWithBeat1Displacement >= 5` (funk) and `burstBars >= 5` (hip-hop) against expected rates of ~19 and ~13–30 respectively. A regression that halves either rate would still pass. Tighten after a 20-run reliability sample anchors the empirical floor. ~30min. *Source: drums-idiom/S6 review (2026-05-18).*
- ✅ CLOSED (already shipped — verified stale 2026-05-28): **Ska-Punk shared-hook reinforcement branch is dead — `sharedHookBuffer` is never populated.** No longer true — `publishSoloistHook` (`soloist.ts:660`) now pushes a `SoloistHook` on every qualifying anchor (added post-2026-05-20), `tick-logic.ts:597` syncs the buffer to `coordination.soloistSharedHookBuffer`, and `harmonies.ts:375` consumes it on `feel === 'Ska'` (rekeyed reachable by meter-robustness S1). The antiphony fires in production. Original entry:  `harmonies.ts` `playShadowMode` has a Ska-Punk branch that echoes hooks the soloist has shared (`coordination.soloistSharedHookBuffer`). But repo-wide, the underlying buffer is only ever reset to `[]` — no code path ever pushes a `SoloistHook` entry, so the branch never fires in production. Pre-existing — S9(b) faithfully rerouted the (empty) buffer through the coordination contract without changing behavior. If Ska-Punk antiphony is intended to function, a separate story needs to write into the buffer (the soloist emitting a hook on a phrase it wants harmony to echo). ~3h if pursued. *Source: Epic 11 S9 review (2026-05-20).*
...
- ✅ SHIPPED — **Epic 3/S12** (2026-05-29, commit `bcb8727b`). **Decoupled** the widen: only `barsUntilSectionChange` opens to the `<= stepsPerMeasure * 2` window (pure function of `remainingSteps`, no `getChordAtStep` call → cursor untouched); the harmonic-lookahead trio stays final-bar-only. Three-tier bass push ramp 1.0/0.5/0.15×. **The "fields cross the worker boundary" premise was wrong** — `coordination` is a per-tick worker-internal scratchpad, never in `getSyncState()`; both reviewers clean. Original entry preserved below. **`barsUntilSectionChange` lookahead window is one bar — no penultimate-bar "approach" tier.** `tick-logic.ts:175-178` only publishes `barsUntilSectionChange` inside the `remainingSteps <= stepsPerMeasure` guard, and floors `(remainingSteps - 1) / stepsPerMeasure` — so the field only ever holds `0` (final bar) or `-1` (default), never `1+`. Epic 11 S2's rock push therefore ships an honest *two-tier* gate (1.0× at boundary / 0.15× residual); a musically nicer three-tier ramp with a 60% "approach window" one bar out is impossible until the lookahead is widened to the penultimate bar. Widening the guard to `<= stepsPerMeasure * 2` would also shift when `upcomingSectionFirstChord` / `upcomingSectionLabel` / the drop mechanic publish — a cross-cutting S1-infrastructure change that needs its own story + listen test. ~2h if pursued. *Source: Epic 11 S2 review (2026-05-20).*
...
- **World/Latin drum-preset bank is unreachable in the UI** → ✅ DOCUMENTED — NO ACTION (owner call 2026-05-29, during Epic 3 S10). The genre picker (`smart-genres.ts` `GENRE_OVERRIDES`, 13 genres) exposes **Bossa** as the only Latin-family option (`feel: 'Bossa Nova'`, `drum: 'Bossa Nova'`). The generic `'Latin'` strategy key (`groove-engine.ts` strategies map) and the `Samba` / `Afro-Cuban 6/8` / `Latin/Salsa` / `Afrobeat` presets (`drum-presets.ts` `category: 'World/Latin'`) are referenced by no genre and rendered by no drum-preset picker — `loadDrumPreset` is only ever called genre-driven; `PresetLibrary` is *chord* progressions. **Owner disposition: intentional curation** — Bossa is the deliberate single Latin-family genre; the rest are dormant-by-design, NOT dead code to delete. They stay ready for if/when the World/Latin bank is exposed (would be a new genre/drummer-style-picker story, deliberately not opened). Documented in-code at the `groove-engine.ts` strategies map so it's not re-flagged. Consequence for S10: the new compound Bembé bell's only live trigger today is a Bossa selection forced to a compound meter. *Source: Epic 3 S10 mid-cycle owner observation (2026-05-29).*
