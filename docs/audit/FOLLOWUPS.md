...
- **Three soloist motivic-fidelity test failures — diagnosed: RNG-stream drift from Epic 10 S2.a.** `tests/integration/soloist-motivic-response.test.ts` ×2 (`blues.aggregate.loop1AnchorExactRate` ~0.667 vs ≥0.7; `blues.loop1AnchorExactRate` ~0.25 vs ≥0.75) and `tests/integration/soloist-seeder-hook-shape.test.ts` ×1 (`headC.richContourShare` ~0.5625 vs ≥0.6). `git bisect` pins the first bad commit to `008b2400` (Epic 10 S2.a). **Mechanism:** S2.a replaced two `Math.random()` calls in the head-bypass jitter (`soloist.ts` ~1140/1173 — the gate + the step-offset draw) with the pure `scrambleHash()`. The soloist analysis sweep (`scripts/soloist-analysis-utils.ts` `withSeededRandom`) pins `Math.random` to **one** seeded mulberry32 stream for the whole sweep, so removing draws shifts every downstream `Math.random()` consumer — the picker that places anchor notes now reads a shifted sequence and produces different pitches. Confirmed: reverting just those two draws to `Math.random()` makes all three tests pass. **Not a production regression** — production `Math.random` is unpinned, and S2.a's deterministic jitter is a genuine looped-playback improvement that should *not* be reverted. The real exposed weakness: anchor head notes are picker-driven and RNG-sensitive, so they do not *robustly* echo the head on loop 1 — the pre-S2.a "pass" was a favorable stream alignment, not a guarantee. **Two fix paths:** (1) test recalibration — re-baseline the blues `loop1AnchorExactRate` thresholds to the post-S2.a stream + document the metric as alignment-sensitive (~1h); (2) engine fix — genuinely protect anchor head notes so they play exact on the loop-1 paraphrase regardless of RNG ("state the tune before you stray"), which makes the metric robust and is the musically correct answer (~3-4h, opus, listen-test-gated). Path (2) is the better story. *Source: Epic 11 S5 cycle diagnosis (2026-05-20).*
