...
- **Jazz 12/8 ride skip-beat critique coverage.** S11 fixed the compound-meter spang-a-lang placement (`groupSteps - 2`); the fix is mechanically meter-agnostic and applies to 12/8 too, but the new `tests/standards/jazz-6-8-ride-position-critique.test.ts` only guards 6/8. Add a 12/8 `describe` (or a sibling file) asserting cluster `{0, 4, 6, 10, 12, 16, 18, 22}` ≥ 90% / off-cluster `{5, 11, 17, 23}` ≤ 5%. Same harness, swap `'6/8'` → `'12/8'`. Size: ~30min. *Source: epic-1-compound-meter S11 review (2026-05-27).*
- **Jazz 6/8 comping chord-change coverage.** S13's `tests/standards/jazz-comping-6-8-critique.test.ts` exercises 64 bars on a single Cmaj7. Jazz comping behavior differs on chord-change bars (new-chord-downbeat anchoring at `accompaniment.ts:1501`, final-bar cadence path). A regression that broke compound comping specifically on chord changes wouldn't be caught. Add a second describe block driving a ii-V-I-VI walk (3 changes per 4-bar phrase) with the same acceptance ranges. Size: ~30min. *Source: epic-1-compound-meter S13 review (2026-05-27).*
- **Soloist phrase-budget cap may be 1 bar short of audit-doc range.** S14's `phraseBarBudget = max(3, min(7, round(3 + (intensity - 0.5) * 8)))` realizes 4-7 bar streaks at intensity 0.7 (mean 4.57, max 6). The audit doc named "8 bars at high intensity." Implementer biased down by 1 to compensate for an `isDownbeat`-only counter (wake-up bar not counted), but the realized values land ~2 bars below the doc midpoint. Either widen the cap (`min(9, ...)`) so intensity 1.0 can sustain 8-bar bursts, or update the audit doc to reflect 4-7. Listen-test driven. Size: ~30min. *Source: epic-1-compound-meter S14 review (2026-05-27).*
- **Soloist budget-forced rest length tops out under 2 bars.** S14's `budgetRestBars = 0.5 + (1 - intensity) * 1.5` gives 0.65-1.85 bars across the intensity range. A Miles-style ballad solo can sit out 4+ bars at quiet sections. Consider quadratic ramp `0.5 + (1 - intensity)^2 * 3.5` so low-intensity rests stretch to 3-4 bars. Defer if no listener complaint. Size: ~30min. *Source: epic-1-compound-meter S14 review (2026-05-27).*
- **Soloist natural-resolution rest length collapses to sub-bar at all intensities.** Existing `restMultiplier = config.restBase * (2.0 - intensity * 1.5)` produces fractional-step rests for jazz (`restBase = 0.05`) clamped to the 3-step `minimumRestSteps` floor — perceptually "barely there." S14 added an intensity-graded floor for budget-forced rests only. Natural-resolution rests (`activeSteps <= 0 + isStrongResolution`) still collapse. Consider applying the S14 intensity-graded floor to all rest entries, not just budget-forced. Size: ~1h. *Source: epic-1-compound-meter S14 implementer notes (2026-05-27).*
- **`_nextActiveSteps` formula doesn't normalize for time signature.** S14 implementer flagged: `Math.floor(baseLength * stepsPerBeat * (0.3 + scrambleHash * 1.2))` in soloist.ts:1758-1760 doesn't divide by `tsConfig.beats`, so 6/8 (stepsPerBeat=2) produces phrases roughly half as long in steps as 4/4 (stepsPerBeat=4), but proportionally similar in beats. Probably correct musically, but if anyone tunes it for "bars," they'll be surprised. Add a comment or normalize explicitly. Size: ~15min. *Source: epic-1-compound-meter S14 implementer notes (2026-05-27).*
- **Scale-degree-aware pickup direction for compound jazz walking bass.** S15's `bass-styles.ts:1338-1342` picks chromatic-below vs chromatic-above at flat 50/50 (matching the 4/4 path). Musically, walking bass picks chromatic-below when resolving from b7 ascending to root, and chromatic-above when resolving from b2 descending to root. A scale-degree-aware variant would read the active scale at the pickup step and prefer the direction whose resulting pitch is a scale tone. ~2h. *Source: epic-1-compound-meter S15 review (2026-05-27).*
- **Approach slot scoring for compound jazz walking bass ignores harmonic info.** S15's `bass-styles.ts:1371-1388` picks 3rd vs 5th on approach slots (mStep 2, 8) by pure stepwise distance to prevMidi. The 3rd carries more harmonic information than the 5th (major/minor identity), and Paul Chambers favors it — but the current scoring will pick the same chord tone on both halves of every bar at high intensity, bordering on monotone. Add a small alternation/seeded jitter, OR a chord-tone-frequency reward so the 3rd wins consistently, OR a hash-seeded 70/30 bias. Add an approach-slot critique assertion when patched. ~1h. *Source: epic-1-compound-meter S15 review (2026-05-27).*
- **Compound-aware pitch picker for non-jazz walking-bass styles.** S15 only patched the `'quarter'` style + jazz feel. Other styles (`bossa`, `funk`, `country`, `rockabilly`, `pop`, `rock backbeat`, `walking-ska`, `slap`) have analogous 4/4-shaped `intBeat === N` picks at `bass-styles.ts:68-69, 195, 543, 602, 673, 694, 798, 868-873, 923, 1210`. They mis-fire in 6/8 the same way the `'quarter'` picker did, but the S12 density gate only routes compound to jazz+quarter, so the broken output is dead unless a future story adds compound idiomatic support per style. Promote to stories per genre when 6/8 idiomatic support is on the agenda. *Source: epic-1-compound-meter S15 implementer paired-site audit (2026-05-27).*
- **Compound-meter `active`-vibe comping ornament 4/4-shaped.** S7 author finding B (2026-05-27): `accompaniment.ts:1100-1107` `generateCompingPattern` jazz/bossa branch's `currentVibe === 'active'` path adds an ornament hit at `getBeatStep(1)`. In 6/8 (stepsPerBeat=2) that lands on mStep 2 — a 4/4 "beat 2" position, not idiomatic in compound. Triggered when soloist just rested OR intensity > 0.75 OR complexity > 0.7. Doesn't violate S13 bounds (pulse-aligned ≥ 70% still passes; 83 hits over 360 bars), but non-idiomatic for slow jazz waltz. Fix: in compound branch, route to `getBeatStep(2)` → mStep 4 (anticipation slot) instead. Size: ~30min. *Source: epic-1-compound-meter S7 review (2026-05-27).*
...
- **Latin Sidestick velocity jitter uses bare `Math.random()`.** `grooves/latin.ts:188` — `velocity = scaleVelocity(0.9, intensity, 0.1) + (Math.random() - 0.5) * 0.1`. Pre-existing (not introduced by S16c), violates the deterministic-phrasing rule the rest of the suite follows. Velocity-only, so low listener impact and it won't shift seeded test draw-counts the way a pitch/rhythm draw would — but it's the last un-seeded entropy source in a touched file. Migrate to a `scrambleHash`-keyed jitter on `(step, 'latin-sidestick-vel')`. Size: ~15min. *Source: epic-1-compound-meter S16c review (2026-05-28) P2-2.*
...
- **Funk slap-bass uses bare `Math.random()` for pop/chuck/hammer gates** in `bass-styles.ts:835, 858, 876, 893`. Each call inside the slap-bass strategy gates a syncopated articulation (pop chord, chuck, hammer-on) and is currently un-seeded — violates the CLAUDE.md "deterministic phrasing" rule and would not survive a future PRNG-migration sweep. Documented in Epic 12 S4 commit `8e76d739` (the WHY pass), but the migration itself is out of scope for that story. ~30-45min to fold into a `scrambleHash` seeded on `(barIndex, sectionId, step, draw-discriminator)` matching the pattern shipped in Epic 12 S1 (soloist) and Epic 12 S4 (`groove-engine.ts` entropy). *Source: Epic 12 S4 music-theory-reviewer (2026-05-23).*
