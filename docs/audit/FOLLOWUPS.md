...
- **`evansIntervals` per-quality test coverage is thin** — `tests/standards/jazz-soloist-authenticity.test.ts` now guards `Dm7 interval-6 < 3.5%` (m7 b5 avoid) and `Cmaj7 interval-6 > 0.5%` (lydian #11 retained). Missing controls that would tighten the per-quality contract: **min6 negative control** (Cm6 interval-6 stays low — the `min6` bucket additionally drops 9), **dom positive control** (C7 retains interval 6 as #11/13), and **alt suppression** (G7alt no longer gets the Evans `{2,5,6,9}` boost — delegated to `alteredHookIntervals`). ~1h to add all three. *Source: Epic 12 S2 review (2026-05-23) P2.*

- **`classifyChordQuality` is case-sensitive.** `quality: 'Major'` / `'Minor'` (capital M) miss the `startsWith('m')` and `startsWith('maj')` branches and fall through to the `'dom'` fallback. Production qualities are all lowercase today, so this is test-fixture-fragile only — but a future test fixture that uses capital-M strings (or a future chord-source that emits them) would silently re-introduce the b5-on-m7 bug. Fix: lowercase-normalize at the top of `classifyChordQuality`, or add explicit `'Major'`/`'Minor'` matches. ~10min. *Source: Epic 12 S2 review (2026-05-23) P2.*
...
- **`jazz-soloist-authenticity.test.ts` doesn't defensively clear `pinnedProfile`.** Sister test relies on auto-rotation visiting all four jazz profiles. If a future test runs `UPDATE_SB { pinnedProfile: 'evans' }` and forgets to dispatch `RESET_STATE` before this test (or test isolation breaks across files), the gate sticky-retains evans and the `expect(profilesSeen.has('bird')).toBe(true)` assertion fails spuriously. Currently green (RESET_STATE in beforeEach covers it), but adding a defensive `dispatch(UPDATE_SB, { pinnedProfile: null })` in beforeEach would harden against future authoring. ~10min. *Source: Epic 12 S3 review (2026-05-23) P2.*
...
- **`ska-punk-shared-hook-critique.test.ts` Restatement phase never witnessed.** The producer test runs only Verse-labeled sections; `deriveSrdcPhase` returns `'statement'` for every tick. The phase-eligibility assertion `expect([...]).toContain(phase)` passes vacuously for restatement (and post-P0-1 patch, also for departure) since those phases are never witnessed. A more complete fixture would include a Verse → Restatement transition (via `sectionContext.isRestatement`) AND a Chorus section to exercise the departure phase, confirming all three eligible phases publish. ~30min. *Source: Epic 12 S10 music-theory-reviewer (2026-05-25) P2-2.*
...
- **`accompaniment.ts` Bossa genre-key audit — partido-alto bank likely dead in production.** Every Bossa-specific branch in `accompaniment.ts` checks `genre === 'Bossa'` (lines 944, 1003, 1252, 1281, 1325, 1334, 2598), but the canonical genre key is `'Bossa Nova'` (verified in `groove-engine.ts:47`, `conductor.ts:54`, `harmonies.ts:241`, `fills.ts:277`, `chords-engine.ts:867`). Combined with the `chords.style === 'jazz'` override at `accompaniment.ts:1218-1219` (Bossa's Smart Genre sets `chord: 'jazz'`), production Bossa flows through `accompaniment.ts` as `genre = 'Jazz'`, and the partido-alto bank shipped in Epic 9 S5.c is probably dead. The Bossa critique tests pass because they inject `'Bossa'` directly into `generateCompingPattern` (test-mock vs production override), so the genre-key drift is invisible from the standards suite. Need a story to (a) audit every `genre === 'Bossa'` branch in `accompaniment.ts`, (b) canonicalize on `'Bossa Nova'` per `feedback_canonical_genre_keys`, (c) trace the `chord: 'jazz'` Smart Genre override and decide whether to route Bossa through its own path or through Jazz with explicit Bossa overrides, (d) re-run Bossa critique tests with canonical keys to confirm the partido-alto bank actually fires. ~3-4h. *Source: Epic 12 S4 music-theory-reviewer (2026-05-23) — surfaced when Bossa was briefly added to `PHRASE_END_THIN_GENRES` and the canonical/short-form key mismatch was exposed; that addition was reverted but the pre-existing asymmetry remains.*
