...
**Deferred Follow-up Sweep (Epic 3, synthesized 2026-05-28) → 🔨 1 / 12 stories.** S1 shipped 2026-05-28 (funk displacement re-weight 50/35/15 + motif-tier floor re-baseline; premise correction — the "sustained 2-bar +2" was already fixed by Epic 12 S6 B5). One-at-a-time owner triage of the open `FOLLOWUPS.md` backlog; 6 decided items found already-shipped (stale open-count block) and dropped; 12 real stories promoted, run via `/cycle`. The one real defect is the acoustic 6/8 snare (Phase 2, S7) — everything else is per-genre correctness/taste the prior cycles deferred.
...
| 3 | [Deferred Follow-up Sweep](epic-3-followup-cleanup.md) | 12 | 1 | **Synthesized 2026-05-28** from a one-at-a-time owner triage of the open `FOLLOWUPS.md` items. **6 decided items were found already-shipped during synthesis** (China 1.0, final-bar hat gate, conductor ramp 0.75/1.25, Ska 0.4 floor, Ska antiphony wire-up — all Epic 12 S6 / meter-robustness S1; FOLLOWUPS open-count block was stale, pre-dating those cycles) and NOT promoted. Phase 1 = mechanical decided-value fixes (funk displacement re-weight + test floors, walking-ska 6th quality-aware, bebop fallback, funk slap docs, odd-meter dub grouping-pulse, Bossa breath gate). Phase 2 = correctness needing a critique test / listen (acoustic 6/8 snare — the one real defect; Imperfect Symmetry low-intensity investigate; comping vibe edge-cases; Latin generic 6/8 bell). Phase 3 = larger/cross-cutting (generic walking-bass target-awareness; penultimate-bar approach window). Run via `/cycle`. **Not started.** |
