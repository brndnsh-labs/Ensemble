...
**Meter-Robustness cycle (Epic 2, synthesized 2026-05-28) → ⏳ 7 / 10 stories. Phase 1 (dead-key) + Phase 2 (determinism/lock) COMPLETE: S1+S2+S3 (genre correctness), S4 (bass density gate) + S5 (comping lanes). Phase 3 nearly done: S6 (minimal.ts) + S7 (ska-punk.ts) shipped — both hoist an isCompound/isPulseStart kick anchor so the filter can't strip the 2nd pulse. Next: S8 acoustic kick (last Phase-3 story), then Phase 4 (S9 bass bossa/dub), Phase 5 (S10 odd-meter sweep).**

| # | Epic | Stories | Done | Notes |
| :- | :- | :-: | :-: | :- |
| 2 | [Meter Robustness + Genre Correctness](epic-2-meter-robustness.md) | 10 | 7 | **Synthesized 2026-05-28** (2 discovery agents + main-thread hot-file triage). Every genre × instrument must groove in any time signature; non-jazz odd/compound meters in scope; correctness not taste. Phase 1 = a verified dead-key cluster (Ska-Punk feel is `'Ska'` but 4 tables key on `'Ska-Punk'`; Metal missing from `FILL_TEMPLATES`; Bossa Nova dead-key in `GENRE_MAP`) — wrong in every meter, single root cause, highest ROI. Phase 2 = determinism/lock (the Epic-1 comp fix applied to the bass density gate + non-jazz comping lanes). Phase 3 = compound groove density (minimal, ska-punk, acoustic kick). Phase 4 = bass bossa/dub 4/4-position math. Phase 5 = odd-meter (5/4,7/4,7/8) degradation sweep. **Not started.** |
