...
**Meter-Robustness cycle (Epic 2, synthesized 2026-05-28) → ⏳ 9 / 10 stories. Phases 1–4 COMPLETE: S1+S2+S3 (genre correctness), S4+S5 (determinism/lock), S6+S7+S8 (compound groove density — kick-pulse-anchor pattern), S9 (bass bossa/dub pulse-derived in compound/odd, both paired sites — gate + note — fixed). Last story: Phase 5 S10 (odd-meter 5/4·7/4·7/8 degradation sweep — opus, may split into a grooves story + a bass story). S9 left a tracked follow-up: odd-meter dub grouping-pulse refinement (FOLLOWUPS §C) is natural to fold into S10's bass half.**

| # | Epic | Stories | Done | Notes |
| :- | :- | :-: | :-: | :- |
| 2 | [Meter Robustness + Genre Correctness](epic-2-meter-robustness.md) | 10 | 9 | **Synthesized 2026-05-28** (2 discovery agents + main-thread hot-file triage). Every genre × instrument must groove in any time signature; non-jazz odd/compound meters in scope; correctness not taste. Phase 1 = a verified dead-key cluster (Ska-Punk feel is `'Ska'` but 4 tables key on `'Ska-Punk'`; Metal missing from `FILL_TEMPLATES`; Bossa Nova dead-key in `GENRE_MAP`) — wrong in every meter, single root cause, highest ROI. Phase 2 = determinism/lock (the Epic-1 comp fix applied to the bass density gate + non-jazz comping lanes). Phase 3 = compound groove density (minimal, ska-punk, acoustic kick). Phase 4 = bass bossa/dub 4/4-position math. Phase 5 = odd-meter (5/4,7/4,7/8) degradation sweep. **Not started.** |
