...
**Deferred Follow-up Sweep (Epic 3, synthesized 2026-05-28) → 🔨 3 / 5 stories** (reconciled from 12 — 7 over-promoted stories were CLOSED-ON-ARRIVAL, already shipped/closed by the archived `epic-followup-drain.md` + Epic 12 S4/S5/S6). Shipped: S1 (funk displacement 50/35/15), S5 (odd-meter dub grouping-pulse), S7 (acoustic 6/8 snare — the one real defect, now fixed). Live: S10 (Latin generic 6/8 bell) + S12 (penultimate-bar approach window) — both opus + listen-gated, paused for owner re-engagement. Lesson: [[verify-followup-against-source]] — synthesis trusted a stale `FOLLOWUPS.md` and matched fix-landed `why:` comments as problem-markers; recency-of-source is the reliable "is it live" signal (every live story traces to the 2026-05-28 meter cycles). Grep for a prior drain/cleanup epic before synthesizing a new follow-up backlog.
