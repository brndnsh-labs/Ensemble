...
### S10 — Audit `beatIndex === N` / `intBeat === N` sites for 5- and 7-beat bars · Model: opus (broad, may split) · ✅ SHIPPED 2026-05-28
- **Approach:** test-driven sweep — a new `odd-meter-degradation-critique.test.ts` renders all 15 drum genres + 6 groove-bearing bass styles in 5/4, 7/4, 7/8 and asserts the acceptance (downbeat present, onsets span ≥ beats-1, no silent core lane; bass per-beat density ≤ 1.3× its own 4/4 baseline). The odd-meter TS configs are sane (`backbeat`/`pulse`/`grouping` per meter), so core timekeeping (keyed on `isBeatStart`/`isBackbeat`/`isPulse`) already degrades gracefully — the `=== N` ornaments mis-fire harmlessly. **Premise correction:** the audit expected widespread breakage; in reality only ONE genuine degradation existed.
- **The one real bug (reviewer-found, NOT in my first pass):** the Latin/Bossa son-clave (`latin.ts` motif 0/1, the structural Sidestick timeline) was keyed on 4/4 step literals (0,6,12 / 4,8) gated only on `!isCompound` — so in 5/4/7/4/7/8 it fired 4/4 positions then **dropped the bar's tail** (5/4 left beats 4-5 silent, 7/4 beats 4-7). Fixed (user-chosen design): derive the clave from the meter's grouping — `isPulseStart` anchors both clave sides, the 3-side adds the pickup (`stepInGroup === groupSteps-2`), the 2-side stays on the pulses. Single-group meters (3/4, 2/4) use `isBeatStart` as the pulse so the 2-side doesn't collapse to a lone downbeat.
- **Two reviewer P0s caught + fixed:** (1) the critique harness lacked an `arranger` slice, so `applyGrooveOverrides` ran a **4/4-internal engine** while iterating odd-meter bars ([[tests-passing-on-wrong-path]] — the whole "no fix needed" verification was unreliable until fixed); mutation-tested the corrected guard catches the reverted clave in every meter it names. (2) the first clave fix regressed 3/4/2/4 (single-group 2-side collapsed to a lone downbeat) — fixed with the `isBeatStart` fallback.
- **Reviewer-confirmed:** the clave is the ONLY structural secondary-lane collapse (disco cowbell + latin shaker are meter-relative density/timbre lanes, verified non-collapsing). Drums + bass otherwise degrade gracefully across all genres. 2 NITs logged inline (pooled sweep doesn't drive non-core perc lane names; 7/8 3-side clave density).
- **Verified:** full suite (2337) + meter-integrity + odd-meter-authenticity green; typecheck + biome clean; 30/30 reliability.
