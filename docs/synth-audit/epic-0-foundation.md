...
**Status:** Shipped 2026-05-21. `InstrumentVoice = 'current' | 'new'` added to all five instrument slices; `SET_INSTRUMENT_VOICE` action routes every module through the existing `instrumentStateMap`. A "New Sound" toggle lives in every instrument's settings group; bass gained an instrument-specific section and its ⚙ gear button (two obsolete `module !== 'bass'` predicates removed). Synth seam: each `synth-*.ts` `play*` is now a dispatcher branching on `voice` → `*Current` (the renamed, bit-identical original body) or `*New` (placeholder delegating to `*Current` until the per-instrument epic fills it in). `voice` round-trips localStorage but is deliberately excluded from the compact share-URL schema. Verified: typecheck + Biome clean, 1975 unit + 52 e2e green; state-discipline-reviewer "safe to land" (0 violations); synth seam self-reviewed (the `synth-graph-reviewer` agent from S2 is not invocable in its own session). Owner confirmed the toggle works and survives a reload.
...
**Status:** Shipped 2026-05-21. New `AudioGraph` / `MasterChain` / `InstrumentBus` interfaces in `types.ts`; the ~23 loose audio-node fields on the `playback` slice collapsed into one `audioGraph: AudioGraph | null`. `initAudio()` builds nodes into typed locals and assembles the graph all-or-nothing — the two `Record<string,GainNode>` casts are gone, and `synth-soloist.ts` / `synth-drums.ts` lost three hidden `(playback as any)` casts (typecheck now covers them). All readers migrated (`conductor`, `scheduler-core`, `synth-*`, `audio-recovery`, `Settings`, `mix-report`). Pure refactor: audio bit-identical. Verified: typecheck + Biome + docs-lint clean, 1975 unit tests green; state-discipline-reviewer "safe to land"; audio-graph hygiene review (via `general-purpose` — `synth-graph-reviewer` loads next session) "behavior-preserving, approved". Owner confirmed playback unchanged.
...
**Status:** Shipped 2026-05-21. Landed as a Schroeder/Freeverb reverb (`public/engine/reverb.ts`), **not** an FDN. The FDN was built first and self-oscillated: Chromium's Web Audio is unstable with cross-coupled `DelayNode` feedback cycles — the unitary mixing matrix that defines an FDN — regardless of (provably sub-unity) loop gain. Diagnosed with an offline-render bisection harness; the Freeverb topology (8 parallel single-loop comb filters → 4 series allpass diffusers) has no cross-coupling and is stable. Two Web Audio traps surfaced and are now defended in code: `lowpass`/`highpass` `Q` is interpreted in **dB** (a small positive Q resonates above unity); and cross-coupled delay feedback. Per-genre presets (`room`/`hall`) selected in `conductor.ts`. Old `createReverbImpulse` convolver removed. Modulation deferred — a static Freeverb avoids the loop-modulation instability; gentle per-comb modulation is a safe follow-up. Permanent regression guard: `tests/e2e/reverb-stability.spec.ts` (offline render, asserts the tail decays). Verified: typecheck + Biome + docs-lint clean, 1974 unit tests green; stability repro passes both presets; synth-graph-reviewer "safe to land" (0 P0/P1). Owner A/B-auditioned on the test server and approved.
...
**Status:** Shipped 2026-05-21. All three mix-side mechanisms addressed. (a) `MIXER_GAIN_MULTIPLIERS.chords` 0.13 → 0.16 (≈+1.8 dB) — chords were sitting under bass (0.1575) and soloist (0.15). (b) The chords-bus 2.5 kHz peaking filter inverted from a −2 dB cut to a +2 dB broad (Q 0.7) presence lift — the old cut scooped the band the ear localizes on; variable renamed `notch` → `presence`. (c) New glue bus compressor on the master chain ahead of the brick-wall limiter (`masterGain → glue → saturator → limiter → destination`): SSL-style −18 dB threshold, 6 dB soft knee, 2:1 ratio, 25 ms attack, 250 ms release — evens full-band peaks so the limiter stops yanking the chord bed down. `MasterChain.glue` added to `types.ts`; mix change applies to the `current` mix (no `new`-voice toggle). Verified: typecheck + Biome clean, 58 affected engine/component tests green (`mix-integrity` master-chain assertion + `engine-bus` gain expectation updated); synth-graph-reviewer "safe to land" (0 P0/P1; one P2 is the pre-existing partial-init pattern, not a regression). Owner A/B-auditioned and approved.
...
**Status:** Shipped 2026-05-21. `humanizeNote(seed, profile, scale)` + `humanizeSeed(step, instrument, voiceIndex)` + `HUMANIZE_PROFILES` added to `synth-utils.ts`. `humanizeNote` returns three independent seeded draws (`timeOffset` s, `velocityMult` ×1.0, `detuneCents`) off `hash-utils.ts` `scrambleHash`; `scale` is the `groove.humanize / 100` knob (0 = no-op). Profiles widen `drums → bass → chords → harmonies → soloist` (drummer tighter than soloist; drums carry no detune). Drums wired as first consumer: `scheduleDrums` humanizes each hit's `playTime`/`velocity` independently, seeded on `(step, piece)`; drums now take un-jittered `swungTime` instead of the shared per-tick `t`. The legacy shared `Math.random()` jitter stays (commented) for bass/chords/harmonies/soloist until Epics 5/2/1/3 migrate each. One hardening fix from review applied (`Number.isFinite` guard on the `humanize` divide). Carry-over noted in `epic-4-drums.md`: frozen `playDrumSoundCurrent` keeps its own un-seeded `velJitter`, so `playDrumSoundNew` must drop it for fully-reproducible drums. Verified: typecheck + Biome clean, 17 `synth-utils` tests (5 new: determinism, per-instrument/voice independence, decorrelation, scale-0 no-op, spread bounds) + 864 engine tests green; synth-graph-reviewer "safe to land" (0 P0/P1). Owner A/B-auditioned and approved.
...
**Status:** Shipped 2026-05-21. `velocityTimbre(velocity, options)` added to `synth-utils.ts` → `{ brightness, cutoffMult, drive }`: clamps velocity 0..1, shapes it through a tunable `curve` exponent (`<1` concave/forgiving, `1` linear, `>1` convex/acoustic — soft notes stay dark, blooms on hard hits), maps linearly into `cutoffRange`/`driveRange`. Worked example: `playBassNoteNew` (was a placeholder delegating to `playBassNoteCurrent`) is now a real two-layer bass — sine sub + sawtooth through a lowpass whose cutoff tracks velocity via `cutoffMult`, into a fixed soft-clip fed by a pre-gain that tracks velocity via `drive`. `playBassNoteCurrent` untouched/bit-identical; Epic 5 ("Bass Finishing") builds the `new` path out further. synth-graph-reviewer found 2 P0 + 1 P1 + 2 P2 — all fixed: NaN guards on `bendStartInterval`/`velocity`/`muteAmount`, bend clamped to ±24 semitones (no zero-anchor exp-ramp), `releaseTime` floored above the decay anchor (envelope events stay ordered), `velocityTimbre` hardened against non-finite `velocity`/`curve`. Verified: typecheck + Biome clean, jscpd 2.16% (< 5%), 48 bass/synth-utils tests (4 new for `velocityTimbre`) + 872 engine tests + funk-bass critique green. Owner A/B-auditioned bass New vs Current and confirmed velocity→brightness.
