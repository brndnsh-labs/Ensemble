...
- **Orphaned Latin content (found 2026-05-22 during the S6 listening gate).** S6's five hand-percussion `new` voices (Clave, Conga/Bongo, Agogo/Perc, Guiro, Shaker) are largely impossible to audition: the dense percussion lanes live in the `World/Latin` drum presets (`Latin/Salsa`, `Samba`, `Afro-Cuban 6/8` in `data/drum-presets.ts`), and those presets — plus the `latin.ts` groove strategy and the `minimal`/`shred` grooves — have **no UI path** from the 13-genre selector (rock/jazz/funk/disco/hip-hop/blues/neo-soul/reggae/acoustic/bossa/country/metal/ska-punk). Reachable today: `Conga` via Bossa Nova (one hit per 2 bars) + 2 fill patterns, `Perc` via disco, `Shaker` via acoustic. `Clave`/`Bongo`/`Guiro` are unreachable. This is a **product gap, out of scope for the synth audit** — surfacing the World/Latin presets / Latin genre is the fix, tracked here as a discovery note. S6's voices are correct-by-construction and will light up once that path exists.
