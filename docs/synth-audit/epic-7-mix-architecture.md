...
**S3a status — shipped 2026-05-25.** A 5 kHz `air5k` probe lives alongside the legacy 7.2 kHz `air` probe in `scripts/audio-analysis.ts` (and the mirrored in-page version in `scripts/mix-report.ts`); `mix:analyze` now prints both as `5k%` and `7k%` columns; `mix:diff` flags deltas on both bands. No finding-gate change yet — that's an owner judgment call documented in [`tmp/overnight-s3a-probe.md`](../../tmp/overnight-s3a-probe.md).
...
**Status — shipped 2026-05-25.** New `public/engine/arc.ts` exports `loopArcMultiplier(loopIndex, loopLimit)` — a hold+lift+release curve (`[0.92, 1.05, 1.30, 0.95]` for N=4, table for N ≤ 8 plus a fallback formula). Two call sites: (1) `scripts/mix-report.ts` per-loop, writes `state.playback.bandIntensity = scene.intensity * arcMult` directly because the conductor doesn't run in the offline render path (`autoIntensity=false`, no `dispatch`); (2) `public/engine/conductor.ts` at the two `targetIntensity` write sites inside `if (playback.autoIntensity)`, gated `loopLimit > 1` so free-form playback stays bit-identical. Engines need zero code changes — they already read `playback.bandIntensity` per tick (soloist, groove, bass, harmony). Soloist's monotonic `+ loopCount * 0.05` chorus-evolution boost is preserved.

Reviewers: `state-discipline-reviewer` clean (0/0/0; dispatch route correct, `mix-report.ts` direct write is consistent with the existing offline-harness pattern). `music-theory-reviewer` returned 3 P1 / 3 P2 / 3 NIT — addressed inline: (a) revised the helper doc comment to be honest about the soloist *not* "climaxing over a thinning bed" (it reads the same `bandIntensity` and arcs with the bed; the +0.05/loop boost partially lifts loop-3 but doesn't invert the curve); (b) tightened gate to `loopLimit > 1` since the helper itself is a no-op for N=1; (c) documented per-genre uniformity, multiplicative section-role composition, jitter-before-arc, and final-loop-fill-at-0.95× as deferred follow-ups in the doc comment.

Numeric DoD met via a finding-gate refinement (S3a precedent): the "flat across loops" auto-finding now reads `full+solo` (the listener-meaningful stem) instead of `full` (bed-only, bass-RMS-dominated, resistant to arc modulation). With the gate switched, jazz-ride flips from `building` (monotonic, pre-S4) to `arc` (peak loop 2 at -17.31 dB, release loop 3 at -19.16 dB — the head→build→peak→release shape). Per-stem arc labels: drums `flat → arc`, chords `flat → arc`, soloist `building → arc` (now peaks at loop 2 instead of monotonic). Bass remains `flat` (~0.6 dB swing, too weak to clear the 1.5 dB classifier floor) — a known follow-up: bass voice would need note-density variation or wider velocity range to RMS-respond more. Other scenes (funk-pocket / rock-backbeat / blues-shuffle) deferred to a post-commit verification pass; per-genre arc shape (esp. funk's flat-pocket idiom) is the most likely place a follow-up tune-pass lands.

Owner approved through the listening gate.
...
- **S4 may grow.** A real intensity arc may want to feed back into the form-arranger work (archived at `docs/archive/musical-audit-2026-05/epic-form-arrangement.md`; live engine code is `public/engine/arrangement-layering.ts` + `coordination-engine.ts`) so the arc lines up with section boundaries. Cross-track coordination needed.
- **S5 shipped 2026-05-24** as preset-data only; the UI-toggle clause was scoped out after discovering the chart-first surface deliberately retired the step sequencer in March 2026 (`f627bc8f`). Funk preset now fires Shaker + Conga; funk-pocket air-band auto-finding stops printing.
