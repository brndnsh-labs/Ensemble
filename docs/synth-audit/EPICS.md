...
**Current position:** Epic 0 complete (7/7, 2026-05-21). **Epic 1 (Harmony Voice Rebuild) complete (6/6, 2026-05-22)** — `playHarmonyNoteNew` rebuilt into a real voice: decoupled style/genreFeel, ADSR, named Horn Section + String Pad formant voices, bus character EQ, hygiene cleanup. Organ branch carried over untouched. **Epic 2 (Chords → Electric Piano) complete (7/7, 2026-05-22)** — the `new` Piano chord voice is a full rebuild: strum-stagger, two-part attack transient, velocity→brightness layer, 9-partial additive body with per-partial decay + pitch-dependent inharmonic stretch, softened polyphony compensation, fail-fast NaN guards. A stop-before-start P0 in the S4 additive body (caught at the S6 listening gate — "one note at a time", every genre) was fixed. **Epic 3 (Soloist Expressiveness) in progress (3/7, 2026-05-22)** — S1: dead legato/portamento path woken via rhythmic adjacency + soft legato attack. S2: all 5 presets couple filter brightness to a blend of per-note velocity and band intensity, gated on `soloist.voice === 'new'` (the soloist "New Sound" toggle is now audible — it was a no-op delegate before). S3: shared `attachCutoffLfo` adds a slow cutoff LFO so sustained notes breathe. S4: the vibrato LFO is tapped 3 ways (pitch+amplitude+cutoff) so the wobble has body; pitch depth widened. S5: seeded per-note timbral humanization (cutoff/detune/attack/bell jitter) so repeated notes aren't byte-identical. S6: attack-time unison detune-settle on every preset; shred detune tightened +12c→+6c. **Next: Epic 3 S7 (articulation-aware ADSR release) — last story in Epic 3.** Resume with `/synth-cycle`.
...
| 3 | [Soloist Expressiveness](epic-3-soloist.md) | 7 | 6 | Highest-risk (most exposed). S1–S6 shipped 2026-05-22 — legato/portamento; velocity+intensity→brightness; cutoff LFO; coupled 3-way vibrato; per-note humanization; attack detune-settle + shred tightened. |
