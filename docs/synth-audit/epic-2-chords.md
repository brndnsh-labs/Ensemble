...
- *Deferred level tune (owner ear, 2026-05-22):* after S6's RMS-normalization fix the owner noted the new chord voice may run **slightly too loud**. Deliberately not re-tuned now — the per-instrument epics (3 soloist, 4 drums, 5 bass) rebuild the rest of the mix, so the relative balance is in flux; a final level pass belongs after more voices are done. The single knob: `BANK_RMS` in `playAdditiveBody` (`synth-chords.ts`, currently `0.72`) — lower it (~0.55–0.62) to bring the body down. The S2 transient (`finalVol*0.45`/`*0.3`) and S3 bright layer (`finalVol*brightness*0.45`) also contribute. Re-check at the post-epic cross-genre listening pass.
