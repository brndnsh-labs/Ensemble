...
- **Follow-up (found during S1 review, out of S1 scope):** both `playHarmonyNoteCurrent` and the new `playHarmonyNoteNew` finite-check `freq` at entry but not `time` or `vol` — a `NaN` `time` survives `Math.max(time, now)` and poisons every `setValueAtTime`; a `NaN` `vol` poisons the gain ramp. Also the organ branch guards `playback.bandIntensity` (`|| 0.5`) while the non-organ Bloom block does not. Pre-existing in both voices, not an S1 regression — fold into S2 (envelope rebuild touches the same entry/`finalVol` region).
- **Follow-up (found during S1 review):** the AR release is an exponential `setTargetAtTime` approach that never reaches 0, and `osc.stop()` fires at `playTime + duration + 0.5` while gain is still ~13% of peak for long-release pads — an audible stop click. Pre-existing in both voices. S2's real ADSR should schedule the oscillator stop after the release has effectively completed.
