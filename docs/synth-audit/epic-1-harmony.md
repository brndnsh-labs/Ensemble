...
**Status:** Shipped 2026-05-22. `playHarmonyNoteNew` is now a real standalone voice — no longer delegating to `*Current`. `style` is decoupled from `genreFeel`: organ is a top-level branch (organ-under-Rock no longer breaks) and non-organ timbre comes from `resolveHarmonyTimbre`, where feel only biases the style's base waveforms. Legato continuation and pitch-aware stealing extracted into New-only helpers (`extendLegatoHarmonyVoice`, `stealHarmonyPitchVoice`) — keeps the body distinct under jscpd. Entry guard now rejects non-positive `freq` (folds in the `harmony.md` §5 Bloom-ramp follow-up). Organ branch carried over unchanged. `synth-graph-reviewer` clean (P0: 0, P1: 0); 4 P2s were all pre-existing debt inherited from `*Current` — captured as S2 follow-ups in `harmony.md` §5. typecheck/Biome/jscpd green, 954 harmony tests pass. Owner auditioned the previously-hijacked style/feel combinations and approved.
...
**Status:** Shipped 2026-05-22. The `style === 'stabs'` path in `playHarmonyNoteNew` is now a real "Horn Section" voice: a sawtooth core, a register-aware brass bell peak (~1.9 kHz) on the body path, two parallel bandpass formants (1.2/2.5 kHz) for the resonant brass honk, a noise-breath layer, and a gentle decay-settle "tiny swell". Two owner-feedback iterations followed the first gate: (1) rapid-stab volume inconsistency — the breath was reshaped from a sustained airy bed into a front-loaded attack chiff scaled by 1/√voices so chord stabs don't accumulate a noise wash, and the decay-settle was eased (85%→92%); (2) an intermittent click on cut-off notes — root-caused to the shared `killActiveVoices` fading with a never-reaching-zero `setTargetAtTime` then hard-stopping nodes at ~3% gain, fixed with a local click-free `killHarmonyVoice` (linear ramp to exactly 0, node stop deferred past the fade). `synth-graph-reviewer` clean across all three passes (P0/P1/P2: 0). A fourth pass caught a canonical-key mismatch — the voice keyed its style branches on `'stabs'`/`'disco'`, strings the engine never emits (the real `HARMONY_STYLES` vocabulary is `horns`/`strings`/`organ`/`plucks`/`counter`), so the Horn Section was dead code; rekeyed all branches (`'stabs'`→`'horns'`, dead `'disco'` removed) plus a New-only key-bridge shim for the shared `getHarmonyRetriggerProfile`. typecheck/Biome/jscpd green, 954 harmony tests pass. Owner auditioned the now-reachable horn voice (funk, harmony isolated, intensity 85%) and approved.
