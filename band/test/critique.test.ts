...
const CLAIMS: Record<StyleId, Claim[]> = {
    ...
    neosoul: [
        ...
        ['ghostsPerBar', 2.5, 4.5, "ghosted e's and a's around the backbeat above quiet energy"],
        ...
        ['compNotesInScale', 0.99, 1, 'every colour tone is in the chord scale (one authority)'],
        ['compShort', 0, 0.4, 'held for beats, not stabbed: only a bar-end strike is an eighth'],
        ['compOffbeatShare', 0.5, 0.9, 'lazily syncopated: re-strikes on the "e"s and "a"s'],
...
const LOW_CLAIMS: Partial<Record<StyleId, Claim[]>> = {
    ...
    neosoul: [
        ...
        ['ghostsPerBar', 0, 0.5, 'a quiet section plays no ghosts: energy is what adds them'],
    ],
};
...
const GUITAR_CLAIMS: Record<StyleId, Claim[]> = {
    ...
    neosoul: [
        ['compDoubleStopShare', 0.5, 0.85, 'Curtis/Isley: pairs picked out of the grip held'],
        ['compColour', 0.2, 0.5, 'the chord hits are extended; the double-stops are guide tones'],
        ['compNotesInScale', 0.99, 1, 'every colour tone is in the chord scale (one authority)'],
        ['compStrikesPerBar', 2, 4.5, 'sparse: a chord hit and a few pairs a bar, no chatter'],
        ['compUpstrokeShare', 0.2, 0.6, 'the sixteenth pendulum: the "e"s and "a"s come up'],
        ['compMeanLowest', 55, 64, 'the middle of the neck, off the bass, under the singer'],
    ],
};
